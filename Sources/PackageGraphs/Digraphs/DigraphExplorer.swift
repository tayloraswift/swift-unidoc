...
@frozen public struct DigraphExplorer<Node> where Node: Identifiable, Node.ID: Sendable {
    @usableFromInline internal let nodes: Nodes

    /// Nodes that have been fully explored. Once ``queued`` becomes
    /// empty again, this will contain every node of interest.
    @usableFromInline internal var visited: [Node.ID: Node]
    /// Nodes that have been discovered, but not explored through.
    @usableFromInline internal var queued: [Node]

    @inlinable public init(nodes: Nodes) {
        self.nodes = nodes
...
extension DigraphExplorer {
    ...
    @inlinable public mutating func explore(node: Node) {
        {
            if  case nil = $0 {
    ...
    @inlinable public mutating func explore(node id: Node.ID) throws {
        try {
            if  case nil = $0 {
    ...
    @inlinable public mutating func conquer(
        by advance: (inout Self, Node) throws -> ()
    ) rethrows -> [Node.ID: Node] {
