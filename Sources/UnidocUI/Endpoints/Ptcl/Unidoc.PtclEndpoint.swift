...
extension Unidoc {
    @frozen public struct PtclEndpoint: Mongo.PipelineEndpoint, Mongo.SingleOutputEndpoint {
...
extension Unidoc.PtclEndpoint: Unidoc.VertexLayer {
    @inlinable public static var docs: Unidoc.ServerRoot { .ptcl }

    @inlinable public static var docc: Unidoc.ServerRoot { .ptcl }

    @inlinable public static var hist: Unidoc.ServerRoot { .ptcl }
}
extension Unidoc.PtclEndpoint: Unidoc.VertexEndpoint {
    ...
    public func success(
        ...
        case .decl(let vertex):
            guard case .protocol = vertex.phylum else {
                route = vertex.route
                break
            }

            let conformers: Unidoc.ConformingTypes = try .init(
                groups: groups,
                bias: vertex.culture,
                with: context
            )
