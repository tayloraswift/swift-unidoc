...
extension Unidoc {
    @frozen public struct PackagesCreatedEndpoint {
        public let query: PackagesQuery<PackageCreated>
        public var batch: [EditionOutput]

        @usableFromInline let date: Timestamp.Date

        @inlinable public init?(date: Timestamp.Date) {
            guard
            let start: UnixDate = .init(utc: date) else {
                return nil
            }
...
extension Unidoc.PackagesCreatedEndpoint {
    static subscript(date: Timestamp.Date) -> URI { Unidoc.ServerRoot.telescope / "\(date)" }
}
