...
extension Unidoc {
    struct PackagesCrawledPage {
        private let dates: [Timestamp.Date: Date]
        private let year: Timestamp.Year

        private init(dates: [Timestamp.Date: Date], year: Timestamp.Year) {
            self.dates = dates
            self.year = year
        }
    }
                ...
                for month: Timestamp.Month in Timestamp.Month.allCases {
                    ...
                    for day: Int32 in month.days(leap: leap) {
                        $0[.li, { $0.class = day == 1 ? "first" : "" }] {
                            ...
                            guard
                            let date: Date = self.dates[id] else {
                                $0[.div] { $0[.p] = label }
                                return
                            }
