...
extension Unidoc {
    @frozen public struct CompleteBuildsEndpoint {
        public let query: CompleteBuildsQuery
        public var value: CompleteBuildsQuery.Output?

        @inlinable public init(query: CompleteBuildsQuery) {
            self.query = query
            self.value = nil
        }
    }
...
extension Unidoc.CompleteBuildsEndpoint {
    static subscript(package: Symbol.Package, page index: Int) -> URI {
...
extension Unidoc.CompleteBuildsEndpoint: HTTP.ServerEndpoint {
    public consuming func response(as format: Unidoc.RenderFormat) -> HTTP.ServerResponse {
        guard
        let output: Unidoc.CompleteBuildsQuery.Output = self.value else {
