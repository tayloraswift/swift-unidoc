...
extension Unidoc {
    @frozen public struct ConsumersEndpoint {
        public let query: ConsumersQuery
        public var value: ConsumersQuery.Output?

        @inlinable public init(query: ConsumersQuery) {
            self.query = query
            self.value = nil
        }
    }
...
extension Unidoc.ConsumersEndpoint: HTTP.ServerEndpoint {
    public consuming func response(as format: Unidoc.RenderFormat) -> HTTP.ServerResponse {
        guard
        let output: Unidoc.ConsumersQuery.Output = self.value else {
