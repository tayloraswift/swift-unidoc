...
extension Unidoc {
    struct RefsPage {
        ...
        private let consumers: Paginated<ConsumersTable>
        private let versions: Paginated<RefsTable>
        private let branches: [VersionState]
        private let aliases: [Symbol.Package]
        private let buildTools: BuildTools
        private let builds: Paginated<CompleteBuildsTable>
        private let realm: RealmMetadata?
        private let ticket: CrawlingTicket<Package>?
...
extension Unidoc.RefsPage {
    private var view: Unidoc.Permissions { self.versions.table.view }
}
...
extension Unidoc.RefsPage: Unidoc.ApplicationPage {
    func main(_ main: inout HTML.ContentEncoder, format: Unidoc.RenderFormat) {
        ...
        if  let repo: Unidoc.PackageRepo = self.package.repo {
            ...
            main[.dl] = Unidoc.PackageRepoDescriptionList.init(
                repo: repo,
        ...
        if !self.branches.isEmpty {
            ...
            main[.table] = Unidoc.RefsTable.init(
        ...
        main[.dl] {
            ...
            $0[.dd] {
                if  let realm: Unidoc.RealmMetadata = self.realm {
                    ...
                    guard self.package.realmAligning else {
                        return
                    }
