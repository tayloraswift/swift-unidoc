...
extension Unidoc {
    @frozen public struct RefsEndpoint {
        public let query: VersionsQuery
        public var value: VersionsQuery.Output?

        @inlinable public init(query: VersionsQuery) {
            self.query = query
            self.value = nil
        }
    }
...
extension Unidoc.RefsEndpoint {
    @inlinable public static subscript(package: Symbol.Package) -> URI {
        Unidoc.ServerRoot.tags / "\(package)"
    }
}
...
extension Unidoc.RefsEndpoint: HTTP.ServerEndpoint {
    public consuming func response(as format: Unidoc.RenderFormat) -> HTTP.ServerResponse {
        guard
        let output: Unidoc.VersionsQuery.Output = self.value else {
        ...
        let prereleaseTool: Unidoc.BuildFormTool = .shortcut(
            buildable: prerelease?.edition.name,
            submitted: submitted.prerelease,
            package: output.package.symbol,
            label: "Build latest prerelease",
            view: view
        )
        let releaseTool: Unidoc.BuildFormTool = .shortcut(
            buildable: release?.edition.name,
