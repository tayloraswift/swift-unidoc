...
extension Unidoc {
    @frozen public struct RulesEndpoint {
        public let query: RulesQuery
        public var value: RulesOutput?

        @inlinable public init(query: RulesQuery) {
            self.query = query
            self.value = nil
        }
    }
...
extension Unidoc.RulesEndpoint {
    @inlinable public static subscript(package: Symbol.Package) -> URI {
        Unidoc.ServerRoot.rules / "\(package)"
    }
}
...
extension Unidoc.RulesEndpoint: HTTP.ServerEndpoint {
    public consuming func response(as format: Unidoc.RenderFormat) -> HTTP.ServerResponse {
        guard
        let output: Unidoc.RulesOutput = self.value else {
            return .error("Query for endpoint '\(Self.self)' returned no outputs!")
        }

        let display: Unidoc.RulesPage = .init(
