...
extension Unidoc {
    @frozen public struct SitemapEndpoint {
        public let query: SitemapQuery
        public var value: SitemapQuery.Output?

        @inlinable public init(query: SitemapQuery) {
            self.query = query
            self.value = nil
        }
    }
...
extension Unidoc.SitemapEndpoint: HTTP.ServerEndpoint {
    ...
    public consuming func response(as _: Unidoc.RenderFormat) -> HTTP.ServerResponse {
        guard
        let output: Unidoc.SitemapQuery.Output = self.value else {
