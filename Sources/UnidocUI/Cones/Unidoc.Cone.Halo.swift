...
extension Unidoc.Cone {
    struct Halo {
        ...
        private var uncategorized: [Unidoc.Scalar]

        private var requirements: [Unidoc.Scalar]
        private var inhabitants: [Unidoc.Scalar]
        private var superforms: [Unidoc.Scalar]
        private var curation: [Unidoc.Scalar]
        private var products: [Unidoc.Scalar]
        private var modules: [Unidoc.Scalar]

        private var extensions: [[Unidoc.ExtensionGroup]]

        private(set) var peerConstraints: [GenericConstraint<Unidoc.Scalar>]
        private(set) var peerList: [Unidoc.Scalar]

        private let decl: Phylum.DeclFlags?
        private let bias: Unidoc.Bias
...
extension Unidoc.Cone.Halo {
    init(
        ...
        apex: borrowing Unidoc.DeclVertex
    ) throws {
...
extension Unidoc.Cone.Halo {
    private mutating func organize(
        ...
        for group: Unidoc.AnyGroup in groups {
            ...
            case .extension(let group):
                ...
                if  let volume: Unidoc.VolumeMetadata = self.context[
                        secondary: group.id.edition
                    ] {
                    module = .init(
                        partisanship: .third(volume.symbol.package),
            ...
            case .curator(let group):
                ...
                let plane: SymbolGraph.Plane = first.plane else {
                    continue
                }
...
extension Unidoc.Cone.Halo: HTML.OutputStreamable {
    static func += (html: inout HTML.ContentEncoder, self: Self) {
        if  let uncategorized: Unidoc.SegregatedBody = .init(
        ...
        if  let modules: Unidoc.IntegratedList = .init(
                items: self.modules,
                with: self.context
            ) {
            html[.section] {
        ...
        if  let products: Unidoc.IntegratedList = .init(
                items: self.products,
                with: self.context
            ) {
            html[.section] {
        ...
        if  let decl: Phylum.DeclFlags = self.decl,
            let body: Unidoc.SegregatedList = .init(
                group: self.superforms,
                with: self.context
            ) {
        ...
        if  let body: Unidoc.SegregatedList = .init(
                group: self.inhabitants,
                with: self.context
            ) {
            html[.section] {
        ...
        if  let body: Unidoc.SegregatedBody = .init(
        ...
        if  let curation: Unidoc.IntegratedList = .init(
                items: self.curation,
                with: self.context
            ) {
        ...
        guard
        let decl: Phylum.DeclFlags = self.decl else {
        ...
        for extensions: [Unidoc.ExtensionGroup] in self.extensions {
            ...
            for group: Unidoc.ExtensionGroup in extensions where !group.isEmpty {
                ...
                guard
                let culture: Unidoc.LinkReference<Unidoc.CultureVertex> else {
                    continue
                }
                ...
                html[.section] {
                    ...
                    let header: Unidoc.ExtensionHeader = .init(
                        extension: group,
                        culture: culture,
                        module: moduleSymbol,
                        bias: self.bias,
