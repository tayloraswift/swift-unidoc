...
extension Unidoc {
    struct Sidebar<Root> where Root: Unidoc.VertexLayer {
        private let volume: Unidoc.VolumeMetadata
        private let origin: Symbol.Module?
        private let nouns: [Unidoc.Noun]
...
extension Unidoc.Sidebar {
    static func package(volume: Unidoc.VolumeMetadata) -> Self {
        .init(volume: volume, origin: nil, nouns: volume.cultures)
    }

    static func product(volume: Unidoc.VolumeMetadata) -> Self {
