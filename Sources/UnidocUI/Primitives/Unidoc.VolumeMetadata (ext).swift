...
extension Unidoc.VolumeMetadata {
    ...
    func subdomain(_ module: Unidoc.Route) -> Unidoc.VertexDomain? {
        let module: Unidoc.VertexDomain.Module = .init(
            name: module.stem.first,
            url: "\(Unidoc.DocsEndpoint[self, module])"
        )
        return .init(volume: self, module: module)
    }

    static func | (self: Unidoc.VolumeMetadata, _: Never?) -> Unidoc.VertexDomain? {
        .init(volume: self)
    }

    static func | (
        self: Unidoc.VolumeMetadata,
        link: Unidoc.LinkReference<Unidoc.CultureVertex>
    ) -> Unidoc.VertexDomain? {
        guard
        let module: Unidoc.VertexDomain.Module = .init(from: link) else {
            return nil
        }
    ...
    static func | (
        ...
        guard
        let module: Unidoc.VertexDomain.Module = .init(from: link.culture) else {
            return nil
        }
