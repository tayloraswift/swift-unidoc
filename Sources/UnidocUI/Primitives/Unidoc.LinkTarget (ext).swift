...
extension Unidoc.LinkTarget {
    ...
    mutating func export(as article: Unidoc.ArticleVertex, base principal: Unidoc.AnyVertex) {
    ...
    static func relative(
        target article: Unidoc.ArticleVertex,
        base principal: Unidoc.AnyVertex
    ) -> Self? {
        guard case .article(let principal) = principal,
        case article.id.edition = principal.id.edition else {
