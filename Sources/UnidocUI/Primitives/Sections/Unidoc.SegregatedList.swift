...
extension Unidoc {
    struct SegregatedList {
        private(set) var recommended: [Unidoc.DeclCard]
        private var discouraged: [Unidoc.DeclCard]
        private var underscored: [Unidoc.DeclCard]
        private var interfaces: [Unidoc.DeclCard]
...
extension Unidoc.SegregatedList {
    ...
    mutating func append(_ card: Unidoc.DeclCard?) {
        card.map { self.append($0) }
    }

    mutating func append(_ card: Unidoc.DeclCard) {
        guard case nil = card.vertex.signature.spis else {
            self.interfaces.append(card)
            return
        }
        guard card.vertex.signature.availability.isGenerallyRecommended else {
