...
extension Unidoc {
    @frozen public struct IdentifiableVertices {
        @usableFromInline let principal: AnyVertex
        @usableFromInline var secondary: [Unidoc.Scalar: AnyVertex]
...
extension Unidoc.IdentifiableVertices: Identifiable {
    @inlinable public var id: Unidoc.Scalar { self.principal.id }
}
extension Unidoc.IdentifiableVertices: Unidoc.VertexContextTable {
    public init(principal: Unidoc.AnyVertex, secondary: borrowing [Unidoc.AnyVertex]) {
    ...
    @inlinable public subscript(_ vertex: Unidoc.Scalar) -> (
