...
extension Unidoc {
    public class IdentifiablePageContext<Table> where Table: Unidoc.VertexContextTable {
        public let canonical: CanonicalVersion?

        private(set) var packages: PackageContext
        private var cache: Cache

        public let media: PackageMedia
        ...
        public required init(
            ...
            let packages: PackageContext = .init(
        ...
        public subscript(vertex id: Unidoc.Scalar) -> Unidoc.LinkReference<Unidoc.AnyVertex>? {
            self.cache[id]
        }

        public subscript(
            culture id: Unidoc.Scalar
        ) -> Unidoc.LinkReference<Unidoc.CultureVertex>? {
        ...
        public subscript(decl id: Unidoc.Scalar) -> Unidoc.LinkReference<Unidoc.DeclVertex>? {
            self.cache[decl: id]
        }
    }
}
extension Unidoc.IdentifiablePageContext: Identifiable {
    public final var id: Table.ID { self.cache.vertices.id }
}
extension Unidoc.IdentifiablePageContext: Unidoc.VertexContext {
    public final var tooltips: Tooltips? { self.cache.tooltips }

    public final var vertices: Table { self.cache.vertices }

    public final var volume: Unidoc.VolumeMetadata { self.cache.volumes.principal }

    public final var repo: Unidoc.PackageRepo? { self.packages.principal?.repo }

    public subscript(package id: Unidoc.Package) -> Unidoc.PackageMetadata? {
        self.packages.metadata[id]
    }

    public final subscript(volume: Unidoc.Edition) -> Unidoc.VolumeMetadata? {
        self.cache.volumes[volume]
    }
}
extension Unidoc.IdentifiablePageContext {
    public final subscript(secondary volume: Unidoc.Edition) -> Unidoc.VolumeMetadata? {
