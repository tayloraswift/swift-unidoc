...
extension Unidoc.VertexContext {
    ...
    func byline(_ locale: ISO.Locale) -> Unidoc.Byline? {
        guard
        let tagged: Timestamp = self.volume.commit?.date?.timestamp else {
            return nil
        }
    ...
    func card(decl id: Unidoc.Scalar) -> Unidoc.DeclCard? {
        ...
        let target: Unidoc.LinkTarget = reference.target else {
            return nil
        }
...
extension Unidoc.VertexContext {
    ...
    func link(decl: Unidoc.Scalar) -> HTML.Link<UnqualifiedPath>? {
        ...
        let path: UnqualifiedPath = .init(splitting: link.vertex.stem) else {
            return nil
        }
    ...
    func link(source file: Unidoc.Scalar, line: Int? = nil) -> Unidoc.SourceLink? {
        ...
        let origin: Unidoc.PackageOrigin = self.repo?.origin else {
            return nil
        }
...
extension Unidoc.VertexContext {
    func load(
        ...
        href attribute: inout Markdown.Bytecode.Attribute
    ) -> String? {
        guard
        let target: Unidoc.LinkTarget = self[vertex: id]?.target else {
            return nil
        }
    ...
    func load(id: Unidoc.Scalar, src _: inout Markdown.Bytecode.Attribute) -> String? {
        guard
        let file: Unidoc.FileVertex = self.vertices[id]?.vertex.file else {
            return nil
        }
