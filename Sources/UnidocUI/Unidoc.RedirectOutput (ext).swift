...
extension Unidoc.RedirectOutput {
    public func response(as format: Unidoc.RenderFormat) -> HTTP.ServerResponse {
        ...
        case .decl(let vertex)?:    Unidoc.DocsEndpoint[
                self.volume,
        ...
        if  let redirect: URI {
            ...
            let context: Unidoc.PeripheralPageContext = .init(
                canonical: nil,
