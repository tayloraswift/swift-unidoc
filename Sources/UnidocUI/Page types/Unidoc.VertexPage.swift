...
extension Unidoc {
    public protocol VertexPage: RenderablePage {
...
extension Unidoc.VertexPage where Self: Unidoc.StaticPage {
    @inlinable public var canonicalURI: URI? { self.context.canonical?.uri }
}
extension Unidoc.VertexPage {
    var volume: Unidoc.VolumeMetadata { self.context.volume }

    public func head(augmenting head: inout HTML.ContentEncoder, format: Unidoc.RenderFormat) {
        let json: JSON = .array {
            $0 {
