...
extension Unidoc {
    public protocol VertexContext: AnyObject {
...
extension Unidoc.VertexContext {
    @inlinable public subscript(
        ...
        get throws {
            guard let vertex: Unidoc.LinkReference<
                Unidoc.CultureVertex
            > = self[culture: id] else {
    ...
    @inlinable public subscript(
        decl id: Unidoc.Scalar
    ) -> Unidoc.LinkReference<Unidoc.DeclVertex> {
        get throws {
            guard let vertex: Unidoc.LinkReference<Unidoc.DeclVertex> = self[decl: id] else {
