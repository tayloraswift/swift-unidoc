extension HTTP
{
    ...
    struct ServerOrigin:Sendable
    {
        ...
        @inlinable public
        init(scheme:Scheme, authority:String)
        {
...
{
    @inlinable public
    static func https(host:String, port:Int = 443) -> Self
    {
        .init(scheme: .https, authority: port == 443 ? host : "\(host):\(port)")
    }

    @inlinable public
    static func http(host:String, port:Int = 80) -> Self
    {
...
extension HTTP.ServerOrigin:CustomStringConvertible
{
    ...
    @inlinable public
    var description:String { "\(self.scheme.name)://\(self.authority)" }
}
