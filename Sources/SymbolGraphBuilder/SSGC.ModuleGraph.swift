...
extension SSGC {
    @_spi(testable) public struct ModuleGraph {
        private let upstreamPackages: [any Identifiable<Symbol.Package>]
        ...
        /// dependencies.
        private let constituents: [NodeIdentifier: [Node]]
        /// Modules that are assumed to be dependencies of every module in the module graph.
        private let substrate: [ModuleLayout]

        let products: [SymbolGraph.Product]
        let cultures: [Node]

        @_spi(testable) public let snippets: [LazyFile]

        private init(
            upstreamPackages: [any Identifiable<Symbol.Package>],
...
extension SSGC.ModuleGraph {
    ...
    static func package(
        ...
        sparseEdges: [(NodeIdentifier, NodeIdentifier)]
    ) throws -> Self {
        ...
        guard
        let dependencies: [PackageNode] = dependencies.sortedTopologically(
            by: directedEdges
        ) else {
        ...
        guard
        let snippetsDirectory: FilePath.Component = .init(sink.snippets) else {
...
extension SSGC.ModuleGraph {
    ...
    func dependenciesUsed(
        among dependencyPins: [SPM.DependencyPin]
    ) throws -> [SymbolGraphMetadata.Dependency] {
        let dependenciesIndexed: SPM.DependencyPins = try .init(indexing: dependencyPins)
        let dependenciesPinned: [
            SymbolGraphMetadata.Dependency
        ] = try self.upstreamPackages.map {
            let pin: SPM.DependencyPin = try dependenciesIndexed($0.id)
            return .init(
                package: .init(scope: pin.location.owner, name: $0.id),
...
extension SSGC.ModuleGraph {
    private func plan(for module: NodeIdentifier) -> [SSGC.ModuleLayout] {
...
extension SSGC.ModuleGraph {
    func symbolDumpCommands(
        scratch: SSGC.PackageBuildDirectory? = nil,
    ...
    ) throws -> [SSGC.Toolchain.SymbolDumpOptions] {
