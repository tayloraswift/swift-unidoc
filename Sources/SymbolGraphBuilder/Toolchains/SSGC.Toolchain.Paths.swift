...
extension SSGC.Toolchain {
    @frozen public struct Paths {
        /// A path to the SwiftPM cache directory to use.
        @usableFromInline let swiftPM: FilePath.Directory?
        /// A path to a Swift toolchain to use, usually ending in `usr`.
        @usableFromInline let usr: FilePath.Directory?

        @inlinable public init(swiftPM: FilePath.Directory?, usr: FilePath.Directory?) {
...
extension SSGC.Toolchain.Paths {
    ...
    public var swiftCommand: String {
        guard
        let usr: FilePath.Directory = self.usr else {
            return "swift"
        }
