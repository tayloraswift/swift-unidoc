...
extension SSGC {
    @_spi(testable) public final class LazyFile {
        /// The amount of time it took to load this file from disk.
        private(set) var loadingTime: Duration
        private var content: [UInt8]?

        /// Absolute path to the file on disk.
        @_spi(testable) public let location: FilePath
        @_spi(testable) public let path: Symbol.File
        @_spi(testable) public let name: String
...
extension SSGC.LazyFile {
    convenience init(location: FilePath, path: Symbol.File) {
    ...
    /// file type.
    convenience init(location: FilePath, root: borrowing SSGC.PackageRoot) {
    ...
    private func time<T>(while body: () throws -> T) rethrows -> T {
...
extension SSGC.LazyFile: SSGC.ResourceFile {
    @_spi(testable) public func read(as _: [UInt8].Type = [UInt8].self) throws -> [UInt8] {
    ...
    @_spi(testable) public func read(as _: String.Type = String.self) throws -> String {
