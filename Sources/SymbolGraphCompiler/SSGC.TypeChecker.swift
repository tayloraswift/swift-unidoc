...
extension SSGC {
    public struct TypeChecker {
        private var declarations: Declarations
        private var extensions: Extensions

        private var allModules: [Symbol.Module]

        public init(threshold: Symbol.ACL = .public) {
...
extension SSGC.TypeChecker {
    public mutating func add(symbols culture: SSGC.SymbolCulture) throws {
        var id: Symbol.Module? = nil
        for part: SSGC.SymbolDump in culture.symbols {
            guard
            let id: Symbol.Module else {
                id = part.culture
                continue
            }

            guard id == part.culture else {
                throw CultureError.init(
                    underlying: SSGC.UnexpectedModuleError.culture(
                        part.culture, in: .init(
    ...
    private mutating func add(
        symbols culture: SSGC.SymbolCulture,
        from id: Symbol.Module
    ) throws {
            ...
            for vertex: SymbolGraphPart.Vertex in part.vertices {
                do {
                    ...
                    case .scalar(let symbol):
                        guard
                        let decl: SSGC.DeclObject = self.declarations.include(
                            vertex,
                            namespace: namespace,
                            culture: id
                        ) else {
        ...
        for decl: SSGC.DeclObject in children.values.joined() {
            ...
            let parent: Symbol.Decl = protocolsByName[parent] else {
                continue
            }
        ...
        for (_, decls): (Int, [SSGC.DeclObject]) in children.sorted(by: { $0.key < $1.key }) {
            for decl: SSGC.DeclObject in decls {
                ...
                let scope: SSGC.DeclObject = self.declarations[visible: scope] else {
...
extension SSGC.TypeChecker {
    ...
    /// modules.
    private mutating func assign(_ relationship: Symbol.RequirementRelationship) throws {
    ...
    private mutating func assign(
        _ relationship: Symbol.MemberRelationship,
        by culture: Symbol.Module
    ) throws {
        guard
        let member: SSGC.DeclObject = self.declarations[visible: relationship.source] else {
            return
        }
...
extension SSGC.TypeChecker {
    private mutating func collectConformance(
        _ conformance: Symbol.ConformanceRelationship,
        by culture: Symbol.Module
    ) throws -> SSGC.DeclObject? {
        guard
        let target: SSGC.DeclObject = self.declarations[visible: conformance.target] else {
            return nil
        }
        ...
        case .scalar(let symbol):
            guard
            let type: SSGC.DeclObject = self.declarations[visible: symbol] else {
                return nil
            }
            ...
            if  case .protocol = type.value.phylum {
                ...
                //  relationship!
                try type.add(
                    superform: Symbol.InheritanceRelationship.init(
                        by: type.id,
                        of: target.id
                    )
                )
        ...
        case .block(let symbol):
            ...
            guard
            let type: SSGC.DeclObject = self.declarations[visible: named.extendee] else {
                return nil
            }
            ...
            guard named.conditions == conditions || { true }() else {
    ...
    private mutating func insert(
        _ relationship: some SuperformRelationship,
        by culture: Symbol.Module
    ) throws {
        guard
        let superform: SSGC.DeclObject = self.declarations[visible: relationship.target],
        let subform: SSGC.DeclObject = self.declarations[visible: relationship.source] else {
            return
        }
...
extension SSGC.TypeChecker {
    private func computeConformance(
        where conditions: Set<Set<GenericConstraint<Symbol.Decl>>>,
        to target: Symbol.Decl,
        of type: SSGC.DeclObject
    ) throws -> Set<GenericConstraint<Symbol.Decl>> {
        do {
    ...
    private mutating func computeConformances(
        of type: SSGC.DeclObject,
        by culture: Symbol.Module
    ) throws {
        for (target, overlapping): (Symbol.Decl, Set<Set<GenericConstraint<Symbol.Decl>>>)
            ...
            try {
                ...
                guard case nil = $0 else {
                    return
                }

                let canonical: Set<GenericConstraint<Symbol.Decl>>
                do {
                    canonical = try self.computeConformance(
                        where: overlapping,
                        to: target,
                        of: type
                    )
                ...
                //  if one does not already exist.
                self.extensions[extending: type, where: canonical].add(
                    conformance: target,
                    by: culture
                )
                $0 = canonical
...
extension SSGC.TypeChecker {
    private mutating func insert(
        _ relationship: Symbol.FeatureRelationship,
        by culture: Symbol.Module
    ) throws {
        guard
        let feature: SSGC.DeclObject = self.declarations[
            visible: relationship.source.feature
        ] else {
            return
        }
        ...
        case .scalar(let symbol):
            ...
            guard heir.id == relationship.source.heir else {
                throw AssertionError.init(message: """
            ...
            guard
            let conformance: Symbol.Decl = feature.scopes.first else {
                throw AssertionError.init(message: """
            ...
            guard feature.scopes.count == 1 else {
                throw AssertionError.init(message: """
            ...
            > = heir.conformances[conformance] else {
...
extension SSGC.TypeChecker {
    public consuming func load(in culture: Symbol.Module) throws -> SSGC.ModuleIndex {
        ...
        let extensions: [SSGC.Extension] = try self.extensions.load(
            culture: culture,
