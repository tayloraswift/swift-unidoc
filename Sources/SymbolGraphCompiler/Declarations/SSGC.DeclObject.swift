...
extension SSGC {
    ...
    final class DeclObject {
        ...
        private(set) var scopes: Set<Symbol.Decl>

        private(set) var value: Decl

        init(
            conditions: Set<GenericConstraint<Symbol.Decl>>,
...
extension SSGC.DeclObject {
    ...
    func assign(scope: Symbol.Decl, by relationship: some NestingRelationship) throws {
        guard relationship.validate(source: self.value.phylum) else {
        ...
        case let existing?:
            guard case .s = self.id.language else {
                fallthrough
            }
    ...
    func add(requirement: Symbol.Decl) throws {
        guard case .protocol = self.value.phylum else {
    ...
    func add(inhabitant: Symbol.Decl) throws {
        guard case .enum = self.value.phylum else {
