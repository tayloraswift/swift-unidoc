...
extension SSGC {
    struct Declarations {
        private let threshold: Symbol.ACL

        private var decls: [Symbol.Decl: DeclObject]
        private var holes: [Symbol.Decl: Int]
...
extension SSGC.Declarations {
    ...
    mutating func include(
        ...
        case .decl(let phylum) = vertex.phylum else {
...
extension SSGC.Declarations {
    ...
    subscript(visible id: Symbol.Decl) -> SSGC.DeclObject? {
        mutating get {
            guard
            let decl: SSGC.DeclObject = self.decls[id] else {
...
extension SSGC.Declarations {
    ...
    func load(culture: Symbol.Module) -> [(Symbol.Module, [SSGC.Decl])] {
        let included: [Symbol.Module: [SSGC.Decl]] = self.decls.values.reduce(into: [:]) {
            guard $1.access >= self.threshold, $1.culture == culture else {
                return
            }
