...
@Suite struct Parsing {
    private static var parser: Markdown.Parser<Markdown.SwiftFlavor> { .init() }
    ...
    ) static func SourcePositions(
        _ source: Markdown.Source,
        _ expected: (line: Int, column: Int)
    ) throws {
        let tree: Markdown.Tree = .init { Self.parser.parse(source) }
        let expected: SourcePosition = try #require(
            .init(
                line: expected.line,
