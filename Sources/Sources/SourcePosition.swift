...
@frozen public struct SourcePosition: Equatable, Hashable, Sendable {
    /// Contains the line number in the upper 20 bits, and
    /// the column index in the lower 12 bits.
    public let bits: UInt32

    @inlinable public init(bits: UInt32) {
        self.bits = bits
    }
}
...
extension SourcePosition {
    @inlinable public static var zero: Self { .init(bits: 0) }
}
extension SourcePosition {
    @inlinable public var line: Int {
        .init(0xffff_f___ & self.bits >> 12)
    }
    @inlinable public var column: Int {
...
extension SourcePosition {
    ...
    /// greater than `4096`.
    @inlinable public init?(line: Int, column: Int) {
