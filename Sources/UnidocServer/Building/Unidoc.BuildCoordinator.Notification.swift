...
extension Unidoc.BuildCoordinator {
    final class Notification: Sendable {
        private let appeared: BSON.Timestamp
        private let request: Unidoc.Edition

        private let producer: CheckedContinuation<Void, any Error>
        private let producerAwaiting: ManagedAtomic<Bool>
        ...
        private func clear(throwing error: any Error) {
            if  self.producerAwaiting.exchange(false, ordering: .relaxed) {
...
extension Unidoc.BuildCoordinator.Notification {
    func match(
        with subscription: __owned Unidoc.BuildCoordinator.Subscription,
        in database: Unidoc.Database,
        registrar: any Unidoc.Registrar
    ) async -> Unidoc.BuildCoordinator.Subscription? {
        let labels: Unidoc.BuildLabels?
        do {
            labels = try await self.match(
                with: subscription.assignee,
                in: database,
                registrar: registrar
            )
        } catch let error {
    ...
    private func match(
        with assignee: Unidoc.Account,
        in database: Unidoc.Database,
        registrar: any Unidoc.Registrar
    ) async throws -> Unidoc.BuildLabels? {
        ...
        guard try await db.pendingBuilds.assignBuild(id: self.request, to: assignee) else {
