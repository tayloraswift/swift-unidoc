...
extension Unidoc {
    public final actor BuildCoordinator {
        nonisolated let id: Symbol.Triple

        private nonisolated let eventQueue: AsyncStream<Event>.Continuation
        private nonisolated let events: AsyncStream<Event>
        private nonisolated let logger: any ServerLogger

        private var subscriptionCounter: UInt
        private var subscriptions: [UInt: Subscription]
...
extension Unidoc.BuildCoordinator {
    public init(id: Symbol.Triple, logger: any Unidoc.ServerLogger) {
    ...
    public func run(registrar: any Unidoc.Registrar, watching db: Unidoc.Database) async {
        async let _: Void = self.pullNotifications(from: db)
...
extension Unidoc.BuildCoordinator {
    ...
    /// cooldown period.
    private nonisolated func pullNotifications(from db: Unidoc.Database) async {
        while true {
            async let cooldown: Void = await Task.sleep(for: .seconds(5))
    ...
    private nonisolated func pullNotification(from db: Unidoc.Database) async throws {
...
extension Unidoc.BuildCoordinator {
    private func matchNotifications(
        from db: Unidoc.Database,
        ...
        for await event: Event in self.events {
            ...
            case .submit(let id, let subscription):
                if  let buffered: Notification = self.notifications {
                    self.notifications = nil
                    self.subscriptions[id] = await buffered.match(
                        with: subscription,
                        in: db,
                        registrar: registrar
                    )
            ...
            case .notify(let notification):
                if  let (id, buffered): (UInt, Subscription) = self.subscriptions.first {
                    self.subscriptions[id] = await notification.match(
                        with: buffered,
                        in: db,
                        registrar: registrar
                    )
            ...
            case .cancel(let id):
                if  let subscription: Subscription = self.subscriptions.removeValue(
                        forKey: id
                    ) {
                    subscription.cancel()
                }
