...
extension Unidoc {
    struct StreamedRequest: Sendable {
        ...
        private init(endpoint: any ProceduralOperation) {
...
extension Unidoc.StreamedRequest {
    init?(from request: __shared HTTP.ServerRequest) {
        ...
        let route: Unidoc.BuildRoute = .init(route) else {
            return nil
        }

        //  Validate content type.
        guard
        case .media(.application(.bson, charset: nil))? = request.headers.contentType else {
            return nil
        }
