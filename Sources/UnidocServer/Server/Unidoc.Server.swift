...
extension Unidoc {
    public final class Server: Sendable {
        ...
        @usableFromInline let options: ServerOptions

        public let db: Database

        private let metricQueue: AsyncStream<MetricPaint>.Continuation,
        metrics: AsyncStream<MetricPaint>

        private let updateQueue: AsyncStream<Update>.Continuation,
...
extension Unidoc.Server {
    @inlinable public var github: (any GitHub.Integration)? { self.options.github }

    @inlinable public var bucket: Unidoc.Buckets { self.options.bucket }
    ...
    private func format(for request: Unidoc.ServerRequest) -> Unidoc.RenderFormat {
        let username: String?
...
extension Unidoc.Server {
    ...
    func run<Plugin>(plugin: Plugin, while active: ManagedAtomic<Bool>) async throws
        ...
        while true {
            ...
            async let cooldown: Void = Task.sleep(for: Plugin.cooldown)
            ...
            do {
                ...
                guard
                let period: Duration = try await plugin.run(in: context) else {
                    break epoch
                }
...
extension Unidoc.Server {
    ...
    func paint() async throws {
        for await paint: Unidoc.MetricPaint in self.metrics {
            ...
            do {
                let db: Unidoc.DB = try await self.db.session()
                try await db.searchbotGrid.count(
                    vertex: paint.vertex,
                    in: paint.volume,
                    as: paint.searchbot,
                    at: paint.time
...
extension Unidoc.Server {
    public func log(event: HTTP.ServerEvent, ip origin: HTTP.ServerRequest.Origin?) {
        let error: any Error
...
extension Unidoc.Server {
    private func clearance(
        by authorization: Unidoc.Authorization
    ) async throws -> HTTP.ServerResponse? {
        guard case .enforced = self.options.access else {
            return nil
        }
        ...
        guard
        let rights: Unidoc.UserRights = try await db.users.validate(user: user) else {
...
extension Unidoc.Server {
    public func reject(request: HTTP.ServerRequest) async throws -> HTTP.ServerResponse? {
        try await self.clearance(by: request.headers.authorization)
    }

    public func delete(request: Unidoc.ServerRequest) async throws -> HTTP.ServerResponse {
        return .resource("Method not allowed\n", status: 405)
    }

    public func get(request: Unidoc.ServerRequest) async throws -> HTTP.ServerResponse {
    ...
    public func put(request: HTTP.ServerRequest, body: [UInt8]) async -> HTTP.ServerResponse {
        guard
        let request: Unidoc.StreamedRequest = .init(from: request) else {
            return .resource("Malformed request\n", status: 400)
        }
    ...
    private func response(
        running operation: Unidoc.AnyOperation,
        for request: Unidoc.ServerRequest
    ) async throws -> HTTP.ServerResponse {
        switch operation {
...
extension Unidoc.Server {
    private func submit(
        update operation: any Unidoc.ProceduralOperation,
        with body: [UInt8] = []
    ) async -> HTTP.ServerResponse {
        await withCheckedContinuation {
            guard case .enqueued = self.updateQueue.yield(
                .init(
                    operation: operation,
                    payload: body,
                    promise: .init($0)
    ...
    private func respond(
        to request: Unidoc.ServerRequest,
        running operation: any Unidoc.InteractiveOperation
    ) async throws -> HTTP.ServerResponse {
        try Task.checkCancellation()
        ...
        run: do {
            let context: Unidoc.ServerResponseContext = .init(
                request: request,
                format: format,
                server: self
            )
