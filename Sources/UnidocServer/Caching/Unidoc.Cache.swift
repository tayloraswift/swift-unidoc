...
extension Unidoc {
    public final actor Cache<Key> where Key: CacheKey {
        nonisolated let assets: FilePath

        private var table: [Key: HTTP.Resource]

        public init(source assets: FilePath) {
            self.assets = assets
            self.table = [:]
        }
    }
...
extension Unidoc.Cache {
    private func load(_ key: Key) throws -> HTTP.Resource {
        try {
