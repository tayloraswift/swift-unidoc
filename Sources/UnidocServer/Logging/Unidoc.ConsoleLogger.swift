...
extension Unidoc {
    public actor ConsoleLogger {
        private nonisolated let events: AsyncStream<LoggableEvent>.Continuation

        private var logs: ServerLog

        public init(events: AsyncStream<LoggableEvent>.Continuation) {
            self.events = events
...
extension Unidoc.ConsoleLogger: Unidoc.ServerLogger {
    public nonisolated func log(_ event: Unidoc.ServerTriggeredEvent) {
        self.events.yield(.server(event))
    }
    public nonisolated func log(_ event: Unidoc.ClientTriggeredEvent) {
    }

    public func handle(_ event: Unidoc.ServerTriggeredEvent) {
        switch event.type {
    ...
    public func handle(_ event: Unidoc.ClientTriggeredEvent) {
    }
    ...
    public subscript(plugin id: String) -> [Unidoc.ServerLog.Message] {
