...
extension Unidoc.ServerLog {
    ...
    @frozen public struct MessageList<Items> where Items: RandomAccessCollection<Message> {
        @usableFromInline let items: Items
        @usableFromInline let now: UnixAttosecond

        @inlinable public init(items: Items, now: UnixAttosecond) {
            self.items = items
            self.now = now
        }
    }
...
extension Unidoc.ServerLog.MessageList: HTML.OutputStreamable {
    @inlinable public static func += (ol: inout HTML.ContentEncoder, self: Self) {
