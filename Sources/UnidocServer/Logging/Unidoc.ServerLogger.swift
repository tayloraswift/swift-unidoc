...
extension Unidoc {
    public protocol ServerLogger: Actor {
        init(events: AsyncStream<LoggableEvent>.Continuation)

        nonisolated func log(_ event: ClientTriggeredEvent)
        nonisolated func log(_ event: ServerTriggeredEvent)
...
extension Unidoc.ServerLogger {
    @inlinable nonisolated func log(
...
extension Unidoc.ServerLogger {
    @inlinable public nonisolated func log(
...
extension Unidoc.ServerLogger {
    @inlinable public static func run<T>(with body: (Self) async throws -> T) async -> T? {
        ...
        async let _: Void = loop.listen(to: stream)
    ...
    @inlinable func listen(to stream: AsyncStream<Unidoc.LoggableEvent>) async {
