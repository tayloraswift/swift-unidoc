...
extension Unidoc.BuilderUploadOperation:Unidoc.BlockingOperation
{
    ...
    func ingest(artifact build:Unidoc.BuildArtifact,
        on server:Unidoc.Server,
        db:Unidoc.DB) async throws -> HTTP.ServerResponse
    {
        ...
        if  let id:Unidoc.Edition = build.edition
        {
            ...
            guard case .success(let labeled) = build.outcome
            else
            {
        ...
        {
            guard case .success(let unlabeled) = build.outcome
            else
            {
