...
extension Unidoc {
    struct ExportOperation: Sendable {
        ...
        private let request: VertexQuery<LookupAdjacent>

        private init(authorization: Authorization, request: VertexQuery<LookupAdjacent>) {
...
extension Unidoc.ExportOperation {
    init(
        authorization: Unidoc.Authorization,
...
extension Unidoc.ExportOperation: Unidoc.InteractiveOperation {
    func load(with context: Unidoc.ServerResponseContext) async throws -> HTTP.ServerResponse? {
        ...
        do {
            let endpoint: Unidoc.ExportEndpoint = .init(
                rateLimit: .init(remaining: remaining),
                query: self.request
            )
