...
extension Unidoc {
    struct WebhookOperation: Sendable {
        ...
        private let hook: UInt

        private init(event: Event, hook: UInt) {
            self.event = event
            self.hook = hook
        }
    }
...
extension Unidoc.WebhookOperation {
    init(json: JSON, from request: __shared HTTP.ServerRequest) throws {
        ...
        if !request.origin.unknown {
            guard case .github_webhook? = request.origin.claimant else {
...
extension Unidoc.WebhookOperation {
    private __consuming func handle(
        installation event: GitHub.WebhookInstallation,
        at time: UnixMillisecond,
        in db: Unidoc.DB
    ) async throws -> HTTP.ServerResponse {
        let user: Unidoc.User = .init(
            githubInstallation: event.installation,
    ...
    private __consuming func handle(
        create event: GitHub.WebhookCreate,
        at time: UnixMillisecond,
        in db: Unidoc.DB
    ) async throws -> HTTP.ServerResponse {
        ...
        let version: SemanticVersion = package.symbol.version(tag: event.ref) else {
