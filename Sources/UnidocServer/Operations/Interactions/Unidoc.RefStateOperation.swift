...
extension Unidoc.RefStateOperation: Unidoc.InteractiveOperation {
    func load(with context: Unidoc.ServerResponseContext) async throws -> HTTP.ServerResponse? {
        ...
        guard
        let _: Int = remaining else {
            return .resource("Inactive or nonexistent API key\n", status: 429)
        }

        guard
        let ref: Unidoc.RefState = try await db.ref(by: self.symbol) else {
        ...
        let report: Unidoc.EditionStateReport = .init(
            id: ref.version.edition.id,
