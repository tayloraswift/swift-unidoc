...
extension Unidoc.UserIndexOperation {
    func perform(on server: Unidoc.Server) async throws -> HTTP.ServerResponse? {
        guard
        let integration: any GitHub.Integration = server.github else {
            return nil
        }

        let restAPI: GitHub.Client<GitHub.OAuth> = .rest(
        ...
        case .sso:
            let user: Unidoc.User = try await restAPI.connect {
                let user: GitHub.User = try await $0.get(
                    from: "/user",
            ...
            guard
            let web: Unidoc.UserSession.Web = secrets.web else {
                return .forbidden("""
        ...
        case .sync:
            ...
            guard
            let current: GitHub.Repo.Owner = memberships.first?.user else {
            ...
            let users: [Unidoc.User] = memberships.compactMap {
                guard case .active = $0.state else {
                    return nil
                }

                return .init(
                    id: .init(type: .github, user: $0.organization.id),
                    level: .guest,
            ...
            guard
            let _: Bool = try await db.users.update(
                access: users.map(\.id),
