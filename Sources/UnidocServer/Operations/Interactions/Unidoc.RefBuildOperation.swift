...
extension Unidoc.RefBuildOperation: Unidoc.RestrictedOperation {
    func load(
        ...
        let edition: Unidoc.EditionMetadata = outputs.edition else {
        ...
        let name: Symbol.PackageAtRef = .init(
            package: outputs.package.symbol,
            ref: edition.name
        )

        switch self.action {
        case .submit:
            _ = try await db.pendingBuilds.submitBuild(
                id: edition.id,
        ...
        case .cancel:
            guard try await db.pendingBuilds.cancelBuild(id: edition.id) else {
