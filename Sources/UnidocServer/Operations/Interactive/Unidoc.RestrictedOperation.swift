...
extension Unidoc {
    ...
    public protocol RestrictedOperation: InteractiveOperation {
...
extension Unidoc.RestrictedOperation {
    public consuming func load(
        ...
        case .enforced:
            ...
            case .web(nil, _):
                guard
                let oauth: GitHub.OAuth = context.server.github?.oauth else {
                    return .error("""
                        Server is running in secure mode with no OAuth capability!\n
                        """)
                }

                let login: Unidoc.LoginPage = .init(
                    client: oauth.client,
                    flow: .sso,
            ...
            guard self.admit(user: rights) else {
