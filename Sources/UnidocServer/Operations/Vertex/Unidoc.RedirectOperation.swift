...
extension Unidoc
{
    struct RedirectOperation<Query>:ExplainableOperation, Sendable
        where Query:Mongo.PipelineQuery, Query.Iteration == Mongo.Single<Unidoc.RedirectOutput>
    {
        let query:Query
    }
}
extension Unidoc.RedirectOperation:Unidoc.InteractiveOperation
{
    ...
    func load(with context:Unidoc.ServerResponseContext) async throws -> HTTP.ServerResponse?
    {
        ...
        guard
        let output:Unidoc.RedirectOutput = try await db.query(with: self.query, on: .nearest)
        else
        {
