...
extension AWS.S3 {
    ...
    @frozen public struct Connection {
        @usableFromInline internal let bucket: AWS.S3.Bucket
        @usableFromInline internal let http1: HTTP.Client1.Connection

        @inlinable internal init(bucket: AWS.S3.Bucket, http1: HTTP.Client1.Connection) {
            self.bucket = bucket
            self.http1 = http1
        }
    }
...
extension AWS.S3.Connection {
    public func put(
        object content: HTTP.Resource.Content,
        ...
        var headers: HTTPHeaders = key?.sign(
            put: body,
            storage: storage,
            bucket: self.bucket,
            path: path
        ...
        let response: HTTP.Client1.Facet = try await self.http1.fetch(
            .init(
                method: .PUT,
                path: path,
                head: headers,
        ...
        case let status:
            throw AWS.S3.RequestError.put(
                status ?? 0, String.init(
    ...
    public func get(path: String, timeout: Duration = .seconds(15)) async throws -> [UInt8] {
        let response: HTTP.Client1.Facet = try await self.http1.fetch(
            .init(
                method: .GET,
                path: path,
    ...
    public func delete(path: String) async throws -> Bool {
        ...
        case let status:
            throw AWS.S3.RequestError.delete(
                status ?? 0, String.init(
                    decoding: response.body,
                    as: Unicode.UTF8.self
                )
