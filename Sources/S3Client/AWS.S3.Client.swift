...
extension AWS.S3 {
    ...
    @frozen public struct Client {
        @usableFromInline let bucket: AWS.S3.Bucket
        @usableFromInline let http1: HTTP.Client1

        @inlinable init(bucket: AWS.S3.Bucket, http1: HTTP.Client1) {
            self.http1 = http1
            self.bucket = bucket
        }
    }
...
extension AWS.S3.Client {
    @inlinable public func connect<T>(
        with body: (AWS.S3.Connection) async throws -> T
    ) async throws -> T {
