extension Markdown {
    @frozen public struct BinaryDecoder {
        @usableFromInline let bytes: [UInt8]
        @usableFromInline var index: Int

        @inlinable internal init(bytes: [UInt8]) {
            self.bytes = bytes
...
extension Markdown.BinaryDecoder {
    @inlinable internal mutating func read() -> UInt8? {
    ...
    @inlinable internal mutating func read<Reference>(_: Reference.Type) -> Int?
        where Reference: FixedWidthInteger & UnsignedInteger {
        if  let after: Int = self.bytes.index(
                self.index,
    ...
    @inlinable internal mutating func read<Instruction>(
        as _: Instruction.Type = Instruction.self
    ) -> Instruction?
...
extension Markdown.BinaryDecoder: IteratorProtocol {
    @inlinable public mutating func next() -> Markdown.Instruction? {
        guard let byte: UInt8 = self.read() else {
            return nil
        }
        switch byte {
