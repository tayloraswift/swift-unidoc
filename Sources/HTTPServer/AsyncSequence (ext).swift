...
extension AsyncSequence where Element: Sendable {
    consuming func forward<T>(
        to channel: AsyncThrowingChannel<T, any Error>,
    ...
    @inlinable consuming func iterate(
        concurrently width: Int,
        with body: @Sendable @escaping (Element) async -> ()
    ) async throws {
        ...
        let exit: Result<Void, any Error> = await withTaskGroup(of: Void.self) {
            ...
            do {
                for _: Int in 0 ..< width {
                    guard
                    let element: Element = try await iterator.next() else {
                        return .success(())
                    }
