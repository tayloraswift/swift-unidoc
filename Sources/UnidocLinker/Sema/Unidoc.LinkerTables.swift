...
extension Unidoc {
    struct LinkerTables {
        private(set) var linker: LinkerContext

        /// A table mapping vertices to topics or autogroups.
        private var group: [Int32: Group]
        private var peers: [Int32: Group]

        private var next: Next

        private(set) var extensions: LinkerTable<Extension>
        private(set) var groups: Mesh.Groups

        private init(linker: consuming LinkerContext, next: Next) {
            self.linker = linker
...
extension Unidoc.LinkerTables {
    ...
    private var modules: ModuleView {
        .init(
            namespaces: self.current.namespaces,
            cultures: self.current.cultures,
            edition: self.current.id
        )
    }

    private mutating func readExtensions() {
        for local: Int32 in self.current.decls.nodes.indices {
            ...
            guard
            let extendee: Unidoc.Scalar = self.linker.current.scalars.decls[local] else {
                let symbol: Symbol.Decl = self.linker.current.decls.symbols[local]
                self.linker.diagnostics[nil] = DroppedExtensionsError.extending(
                    symbol,
...
extension Unidoc.LinkerTables {
    borrowing func linkConformingTypes() -> Unidoc.LinkerTable<Unidoc.Conformers> {
        var conformingTypes: Unidoc.LinkerTable<Unidoc.Conformers> = [:]

        for (id, `extension`): (
                Unidoc.ExtensionSignature,
                Unidoc.Extension
            ) in self.extensions {
            guard
            let extendedType: SymbolGraph.Decl = self.linker[decl: id.extendee] else {
                continue
            }
            /// A lot of C types have `Equatable` and `Hashable` conformances, but we don’t
            /// care about them.
            guard case .swift = extendedType.language else {
                continue
            }
    ...
    mutating func linkProducts() -> [Unidoc.ProductVertex] {
        ...
        var products: Unidoc.CuratorGroup = .init(
            id: self.next(.curator),
        ...
        for (p, product): (Int32, SymbolGraph.Product) in zip(
            ...
            let product: Unidoc.ProductVertex = .init(
                id: self.current.id + p,
        ...
        let cultures: Unidoc.CuratorGroup = .init(
            id: self.next(.curator),
            scope: self.current.id.global,
        ...
        for module: ModuleContext in self.modules {
            for decls: SymbolGraph.Namespace in module.culture.namespaces {
                for (d, node): (Int32, SymbolGraph.DeclNode) in zip(
                    ...
                    let decl: SymbolGraph.Decl = node.decl else {
                        continue
                    }
                    ...
                    for local: Int32 in [decl.requirements, decl.inhabitants].joined() {
                        ...
                        guard
                        let id: Unidoc.Scalar = self.current.scalars.decls[local] else {
                            continue
                        }
                    ...
                    if !intrinsicMembers.isEmpty {
                        self.groups.intrinsics.append(
                            .init(
                                id: intrinsicGroup,
                                culture: module.id,
                                scope: owner,
                                items: self.linker.sort(
                                    intrinsicMembers,
    ...
    mutating func linkArticles() -> [Unidoc.ArticleVertex] {
    ...
    mutating func linkDecls() -> [Unidoc.DeclVertex] {
        self.modules.reduce(into: []) {
            ...
            for colony: SymbolGraph.Namespace in $1.culture.namespaces {
                ...
                guard
                let id: Unidoc.Scalar = self.current.scalars.modules[colony.index] else {
                    self.linker.diagnostics[nil] = DroppedExtensionsError.extending(
                        name,
                ...
                for local: Int32 in colony.range {
                    guard
                    let vertex: Unidoc.DeclVertex = self.linkDecl(
                        at: local,
                        under: namespace
                    ) else {
                        continue
    ...
    mutating func linkCultures() -> [Unidoc.CultureVertex] {
        self.modules.map {
            var vertex: Unidoc.CultureVertex = .init(
                id: $0.id,
    ...
    private func linkRedirect(
        target local: Int32,
        hashed: Bool,
        from namespace: Symbol.Module
    ) -> Unidoc.RedirectVertex? {
        let symbol: Symbol.Decl = self.current.decls.symbols[local]

        guard
        let id: Unidoc.Scalar = self.current.scalars.decls[local],
        let decl: SymbolGraph.Decl = self.linker[decl: id] else {
            return nil
        }
...
extension Unidoc.LinkerTables {
    private mutating func linkArticle(
        at local: Int32,
        under namespace: ModuleContext
        ...
        var vertex: Unidoc.ArticleVertex = .init(
            id: id,
    ...
    private mutating func linkDecl(
        at local: Int32,
        under namespace: ModuleNamespace
    ) -> Unidoc.DeclVertex? {
        let symbol: Symbol.Decl = self.current.decls.symbols[local]
        let node: SymbolGraph.DeclNode = self.current.decls.nodes[local]
        ...
        for s: Unidoc.Scalar in superforms {
            let unconditional: Unidoc.ExtensionConditions = .init(
                constraints: [],
        ...
        var vertex: Unidoc.DeclVertex = .init(
            id: id,
            flags: .init(
