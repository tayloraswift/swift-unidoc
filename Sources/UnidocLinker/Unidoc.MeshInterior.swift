...
extension Unidoc.MeshInterior {
    init(
        primary metadata: SymbolGraphMetadata,
        ...
        let landingVertex: Unidoc.LandingVertex = .init(
            id: current.global,
            snapshot: .init(
                abi: metadata.abi,
    ...
    private init(
        around landing: consuming Unidoc.LandingVertex,
        ...
        for (c, culture): (Int, SymbolGraph.Culture) in zip(
            ...
            guard
            let range: ClosedRange<Int32> = culture.decls else {
                continue
            }
        ...
        for (n, node): (Int32, SymbolGraph.DeclNode) in zip(
            ...
            if  case nil = node.decl,
                ...
                let mirror: Unidoc.ForeignVertex = mapper.register(
                    foreign: vertex,
                    with: linker,
                    as: next
                ) {
        ...
        for (signature, `extension`): (Unidoc.ExtensionSignature, Unidoc.Extension)
            ...
            let assembled: Unidoc.ExtensionGroup = `extension`.assemble(
                signature: signature,
                with: linker
            )
        ...
        for (signature, conformers): (Unidoc.ConformanceSignature, Unidoc.Conformers)
            ...
            let assembled: Unidoc.ConformerGroup = conformers.assemble(
                signature: signature,
                with: linker
            )
            ...
            let next: Int = foreign.count
            if  let vertex: Unidoc.ForeignVertex = mapper.register(
                    foreign: assembled.scope,
                    with: linker,
                    as: next
                ) {
