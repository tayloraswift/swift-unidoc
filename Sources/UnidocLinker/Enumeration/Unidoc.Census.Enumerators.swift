...
extension Unidoc.Census {
    struct Enumerators {
        private(set) var cultures: [Enumerator]
        private(set) var combined: Enumerator
...
extension Unidoc.Census.Enumerators {
    mutating func count(
        ...
        let coverage: WritableKeyPath<Unidoc.Stats.Coverage, Int> = .classify(
            decl,
            _from: snapshot,
            _at: local
        )
    ...
    mutating func count(feature decl: SymbolGraph.Decl, culture: Int) {
        let phylum: WritableKeyPath<Unidoc.Stats.Decl, Int> = .classify(decl)
