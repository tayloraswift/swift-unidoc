...
extension Unidoc {
    struct LinkerContext/*:~Copyable https://github.com/apple/swift/issues/72136*/ {
        ...
        private let byPackageName: [Symbol.Package: LinkableGraph]
        private let byPackage: [Package: LinkableGraph]
...
extension Unidoc.LinkerContext {
    mutating func link(
        pinnedDependencies: [Unidoc.EditionMetadata?],
        ...
        let interior: Unidoc.MeshInterior = .init(
            primary: self.current.metadata,
            pinned: pinnedDependencies.compactMap(\.?.id.package),
            with: &self
        )

        let metadata: Unidoc.VolumeMetadata = .init(
            id: self.current.id,
    ...
    consuming func status() -> DiagnosticMessages {
        let diagnostics: Diagnostics<Unidoc.Symbolicator> = self.diagnostics
        let symbols: Unidoc.Symbolicator = .init(
            context: self,
...
extension Unidoc.LinkerContext {
    private subscript(dynamic package: Symbol.Package) -> Unidoc.LinkableGraph? {
...
extension Unidoc.LinkerContext {
    ...
    func expand(_ scalar: Unidoc.Scalar, to length: Int) -> [Unidoc.Scalar] {
        ...
        for _: Int in 1 ..< max(1, length) {
            ...
            case nil = seen.update(with: next) else {
                break
            }
...
extension Unidoc.LinkerContext {
    private func boundaries(pinned: [Unidoc.EditionMetadata?]) -> [Unidoc.Mesh.Boundary] {
        ...
        for (dependency, pin): (SymbolGraphMetadata.Dependency, Unidoc.EditionMetadata?) in zip(
            ...
            boundaries.append(
                .init(
                    ...
                    targetABI: pinABI,
                    target: .init(
                        exonym: dependency.package.name,
...
extension Unidoc.LinkerContext {
    mutating func link(article: SymbolGraph.Article) -> (Unidoc.Passage?, Unidoc.Passage?) {
    ...
    private mutating func expand(_ outline: SymbolGraph.Outline) -> Unidoc.Outline {
        switch outline {
        ...
        case .symbol(let id):
            guard let id: Unidoc.Scalar = self.current.scalars.decls[id] else {
                return .fallback(nil)
            }
