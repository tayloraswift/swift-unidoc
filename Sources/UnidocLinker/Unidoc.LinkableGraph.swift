...
extension Unidoc {
    @dynamicMemberLookup @usableFromInline final class LinkableGraph {
        let id: Edition
        ...
        private let qualifiers: [Int32: Int]
        /// Maps nested declarations to scopes. Scopes might be non-local.
        private let hierarchy: [Int32: Scalar]
...
extension Unidoc.LinkableGraph {
    convenience init(
        _ object: SymbolGraphObject<Unidoc.Edition>,
        ...
        for (n, node): (Int32, SymbolGraph.DeclNode) in zip(
            ...
            guard let n: Unidoc.Scalar = scalars.decls[n] else {
                continue
            }
...
extension Unidoc.LinkableGraph {
    private func namespace(of declaration: Int32) -> Symbol.Module? {
        self.qualifiers[declaration].map { self.namespaces[$0] }
    }

    private func ancestor(of declaration: Int32) -> Unidoc.Scalar? {
