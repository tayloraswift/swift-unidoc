...
struct Linker
{
    ...
    public
    init(nominations:Compiler.Nominations, metadata:SymbolGraph.Metadata)
    {
        ...
        at address:ScalarAddress) throws -> [ScalarAddress]
    {
        try features.map
        {
            ...
            if  let (last, phylum):(String, ScalarPhylum) =
                self.graph.nodes[feature]?.scalar.map({ ($0.path.last, $0.phylum) }) ??
...
extension Linker
{
    ...
    public mutating
    func allocate(scalars:[Compiler.Culture]) throws -> [[ScalarAddress]]
    {
        try scalars.map
        {
    ...
    func allocate(extensions:[Compiler.Extension]) throws -> [(ScalarAddress, Int)]
    {
        ...
        return try zip(addresses, extensions).map
        {
            ...
            let index:Int = self.graph.nodes[allocated: $0.0].push(.init(
...
{
    public mutating
    func link(scalars:[Compiler.Culture], at addresses:[[ScalarAddress]]) throws
    {
        for (addresses, culture):([ScalarAddress], Compiler.Culture) in zip(addresses, scalars)
        {
            try self.link(scalars: culture.scalars, at: addresses)
        }
    }
    public mutating
    func link(scalars:[Compiler.Scalar], at addresses:[ScalarAddress]) throws
    {
        for (address, scalar):(ScalarAddress, Compiler.Scalar) in zip(addresses, scalars)
        {
...

            {
                ...
                $0?.location = location
                $0?.article = article
            } (&self.graph.nodes[allocated: address].scalar)
        }
    }
    public mutating
    func link(extensions:[Compiler.Extension], at addresses:[(ScalarAddress, Int)]) throws
    {
            ...
            in zip(addresses, extensions)
        {
            ...
            if  let comment
            {
                ...
                self.graph.nodes[allocated: address].extensions[index].article =
