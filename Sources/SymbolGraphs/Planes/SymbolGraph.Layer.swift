extension SymbolGraph {
    @frozen public struct Layer<Node> where Node: SymbolGraphNode {
        public var symbols: Table<Node.Plane, Node.ID>
        public var nodes: Table<Node.Plane, Node>
...
extension SymbolGraph.Layer {
    ...
    @inlinable public mutating func append(_ node: Node, id: Node.ID) -> Int32 {
...
extension SymbolGraph.Layer {
    @inlinable public subscript(scalar: Int32) -> Node? {
        self.nodes.indices.contains(scalar) ? self.nodes[scalar] : nil
    }

    @inlinable public func contains(citizen scalar: Int32) -> Bool {
        self.nodes.indices.contains(scalar) &&
        self.nodes[scalar].isCitizen
    }

    @inlinable public var citizens: Citizens {
        .init(self)
    }
}
extension SymbolGraph.Layer {
    @inlinable public func link<T>(
        _ transform: (Node.ID, Int32) throws -> T,
        dynamic link: (Node.ID) throws -> T
    ) rethrows -> SymbolGraph.Table<Node.Plane, T> {
