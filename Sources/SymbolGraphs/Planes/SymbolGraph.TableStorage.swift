...
extension SymbolGraph {
    ...
    @frozen @usableFromInline internal struct TableStorage<Element> {
        @usableFromInline internal var elements: [Element]

        @inlinable internal init(elements: [Element] = []) {
...
extension SymbolGraph.TableStorage {
    @inlinable internal mutating func reserveCapacity(_ capacity: Int) {
    ...
    @inlinable internal mutating func append(_ element: __owned Element) -> Int32 {
...
extension SymbolGraph.TableStorage: ExpressibleByArrayLiteral {
    @inlinable internal init(arrayLiteral: Element...) {
...
extension SymbolGraph.TableStorage: Sequence {
    @inlinable internal func withContiguousStorageIfAvailable<Success>(
...
extension SymbolGraph.TableStorage: RandomAccessCollection {
    ...
    @inlinable internal var startIndex: Int32 {
        .init(self.elements.startIndex)
    }
    @inlinable internal var endIndex: Int32 {
        .init(self.elements.endIndex)
    }
    @_semantics("array.subscript")
    @inlinable internal subscript(index: Int32) -> Element {
        _read {
...
extension SymbolGraph.TableStorage {
    @inlinable internal subscript(index: Int) -> Element {
        _read {
...
extension SymbolGraph.TableStorage: BSONEncodable where Element: BSONEncodable {
    @usableFromInline internal func encode(to field: inout BSON.FieldEncoder) {
        self.elements.encode(to: &field)
    }
}
extension SymbolGraph.TableStorage: BSONDecodable where Element: BSONDecodable {
    @inlinable internal init(bson: BSON.AnyValue) throws {
