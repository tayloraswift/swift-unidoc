extension Symbol {
    ...
    @frozen public enum USR: Hashable, Equatable, Comparable, Sendable {
...
extension Symbol.USR {
    private init?(_ string: Substring) {
        guard
        let l: String.Index = string.indices.first else {
            return nil
        }
        ...
        let language: Symbol.Decl.Language = .init(language) else {
            return nil
        }

        let start: String.Index = string.index(after: colon)

        guard start < string.endIndex else {
            return nil
        }
        ...
        if  case .s = language,
            ...
            guard
            let feature: Symbol.Decl = .init(language, string[start ..< i]) else {
                return nil
            }
