extension Symbol.Decl {
    @frozen public struct Language: Equatable, Hashable, Sendable {
        public let ascii: UInt8

        /// Creates a language code from an ASCII scalar. Traps if the scalar is not ASCII.
        @inlinable public init(ascii: UInt8) {
...
extension Symbol.Decl.Language {
    ...
    /// C.
    @inlinable public static var c: Self { .init(ascii: 0x63) }

    /// Swift.
    @inlinable public static var s: Self { .init(ascii: 0x73) }
}
extension Symbol.Decl.Language: CustomStringConvertible {
    @inlinable public var description: String { .init(Unicode.Scalar.init(self.ascii)) }
}
