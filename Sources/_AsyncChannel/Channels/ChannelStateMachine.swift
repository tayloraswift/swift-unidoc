...
struct ChannelStateMachine<Element: Sendable, Failure: Error>: Sendable {
    ...
    mutating func send() -> SendAction {
        ...
        case .channeling(
                let suspendedProducers,
                let cancelledProducers,
                var suspendedConsumers,
                let cancelledConsumers
            ):
    ...
    mutating func sendSuspended(
        ...
        switch self.state {
        case .channeling(
                var suspendedProducers,
                var cancelledProducers,
                var suspendedConsumers,
                let cancelledConsumers
            ):
            let suspendedProducer = SuspendedProducer(
                id: producerID,
    ...
    mutating func nextSuspended(
        ...
        case .channeling(
                ...
                let cancelledProducers,
                var suspendedConsumers,
                var cancelledConsumers
            ):
            let suspendedConsumer = SuspendedConsumer(
                id: consumerID,
                continuation: continuation
            )
