...
@Suite struct SymbolQueries: Unidoc.TestBattery {
    ...
    func run(with db: Unidoc.DB) async throws {
        ...
        for (path, expected): (ArraySlice<String>, [String: Int]) in [
            ...
            let lengths: [String: Int] = overview.outlines.reduce(into: [:]) {
                guard
                case .path(let text, let path) = $1 else {
                    Issue.record()
                    return
                }
        ...
        try await self.test(
            "RealmContainingStandardLibrary",
            running: .init("swift", ["swift", "array"]),
            expect: [
                "Sequence",
                "Collection",
    ...
    private func test(
        ...
        for group: Unidoc.AnyGroup in output.principalGroups {
            ...
            case .curator(let t):
                guard filter.contains(.curators) else {
                    continue
                }
            ...
            case .extension(let e):
                guard filter.contains(.extensions) else {
                    continue
                }
            ...
            case .intrinsic(let i):
                guard filter.contains(.intrinsics) else {
                    continue
                }
