...
extension GitHub {
    @frozen public struct UserInvite: Identifiable, Equatable, Sendable {
        ...
        @inlinable public init(id: UInt32, login: String, icon: String, node: Node) {
            self.id = id
            self.login = login
            self.icon = icon
            self.node = node
...
extension GitHub.UserInvite: JSONObjectDecodable {
    public init(json: JSON.ObjectDecoder<GitHub.User.CodingKey>) throws {
        self.init(
