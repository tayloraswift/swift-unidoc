...
extension Markdown {
    ...
    open class BlockElement: TreeElement {
        @inlinable public init() {
        }

        /// Renders this element to bytecode.
        @inlinable open func emit(into _: inout Markdown.BinaryEncoder) {
        }
        ...
        @inlinable open func outline(by _: (Markdown.AnyReference) throws -> Int?) rethrows {
        }
...
extension Markdown.BlockElement {
    ...
    @inlinable public final func rewrite(by rewrite: (inout [Markdown.BlockElement]) -> ()) {
    ...
    @inlinable public final func visit<Block>(
        only _: Block.Type = Block.self,
        with visit: (Block) throws -> ()
    ) rethrows
