...
@Suite struct LinkResolution {
    private static let source: Markdown.Source = .init(origin: nil, text: "")

    private static func _string(_ string: String) -> Markdown.SourceString {
        .init(source: .init(range: nil, in: source), string: string)
    }

    @Test static func IntraModule() {
        var tables: SSGC.Linker.Tables = .init()

        tables.packageLinks["ThisModule", .init(["A"], "b")].append(
            .init(
                traits: .init(
                    autograph: nil,
        ...
        tables.packageLinks["ThisModule", .init(["A"], "c")].append(
            .init(
                traits: .init(
                    autograph: nil,
                    phylum: .func(.instance),
    ...
    @Test static func CrossModule() {
        var tables: SSGC.Linker.Tables = .init()

        tables.packageLinks["OtherModule", .init(["A"], "b")].append(
            .init(
                traits: .init(
                    autograph: nil,
                    phylum: .func(.instance),
        ...
        tables.packageLinks["OtherModule", .init(["A"], "c")].append(
            .init(
                traits: .init(
                    autograph: nil,
                    phylum: .func(.instance),
    ...
    @Test static func IntraModuleArticles() {
        var tables: SSGC.Linker.Tables = .init()
    ...
    @Test static func CrossModuleArticles() {
        var tables: SSGC.Linker.Tables = .init()
    ...
    @Test static func SignatureDisambiguation() {
        var tables: SSGC.Linker.Tables = .init()
        ...
        for (i, (name, (inputs, output))): (Int, (String, ([String], [String]))) in [
            ...
            tables.packageLinks["ThisModule", .init(["A"], name)].append(
                .init(
                    traits: .init(
    ...
    @Test static func RequirementDisambiguation() {
        var tables: SSGC.Linker.Tables = .init()
        ...
        tables.resolving(
            ...
            #expect(
                nil == $0.outline(
                    reference: .lexical(
                        ucf: Self._string(
                            "f(_:) (String) -> () [static func, requirement: false]"
                        )
                    )
                )
            )
