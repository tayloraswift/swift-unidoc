...
extension Unidoc {
    @frozen public struct PackagesCrawledQuery {
        @usableFromInline let range: Range<UnixDate>

        @inlinable public init(during range: Range<UnixDate>) {
            self.range = range
        }
    }
}
extension Unidoc.PackagesCrawledQuery: Mongo.PipelineQuery {
    public typealias Iteration = Mongo.SingleBatch<Date>

    @inlinable public var collation: Mongo.Collation { .simple }
    @inlinable public var from: Mongo.Collection? { Unidoc.DB.CrawlingWindows.name }
