...
extension Unidoc.DB {
    ...
    public func package(named symbol: Symbol.Package) async throws -> Unidoc.PackageMetadata? {
    ...
    /// canonical name than the one provided.
    public func realm(named symbol: String) async throws -> Unidoc.RealmMetadata? {
        try await self.query(
    ...
    public func edition(
        named symbol: Symbol.PackageAtRef
    ) async throws -> Unidoc.EditionOutput? {
        try await self.query(
            with: Unidoc.EditionMetadataSymbolicQuery.init(
    ...
    public func ref(by symbol: Symbol.PackageAtRef) async throws -> Unidoc.RefState? {
        try await self.query(
            with: Unidoc.RefStateSymbolicQuery.init(
                package: symbol.package,
                version: .name(symbol.ref)
            )
        )
    ...
    public func ref(
        of selector: Unidoc.BuildSelector<Unidoc.Package>
    ) async throws -> Unidoc.RefState? {
        switch selector {
        case .id(let id):
...
extension Unidoc.DB {
    public func redirect(
        exported vertex: Unidoc.VertexPath,
        from volume: Unidoc.Edition
    ) async throws -> Unidoc.RedirectOutput? {
        try await self.query(
            with: Unidoc.RedirectByExportQuery.init(
                volume: volume,
                vertex: vertex
            ),
            on: .nearest
    ...
    public func redirect(
        visited vertex: Unidoc.VertexPath,
        in package: Unidoc.Package
    ) async throws -> Unidoc.RedirectOutput? {
        guard
