...
extension Unidoc {
    @frozen public struct RedirectBySymbolicHintQuery<Predicate>: Equatable, Hashable, Sendable
        where Predicate: VertexPredicate {
        public let volume: VolumeSelector
        public let vertex: Predicate

        @inlinable public init(volume: VolumeSelector, lookup vertex: Predicate) {
            self.volume = volume
            self.vertex = vertex
        }
    }
...
extension Unidoc.RedirectBySymbolicHintQuery: Mongo.PipelineQuery {
    public typealias Iteration = Mongo.Single<Unidoc.RedirectOutput>
