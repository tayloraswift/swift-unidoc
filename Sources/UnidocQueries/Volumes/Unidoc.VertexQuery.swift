...
extension Unidoc {
    ...
    @frozen public struct VertexQuery<
        Context
    >: Sendable where Context: LookupContext & Sendable {
        public let volume: VolumeSelector
        public let vertex: VertexPath
        public let lookup: Context
        @usableFromInline let fields: VertexProjection
...
extension Unidoc.VertexQuery<Unidoc.LookupLimited> {
    @inlinable public init(volume: Unidoc.VolumeSelector, vertex: Unidoc.VertexPath) {
...
extension Unidoc.VertexQuery<Unidoc.LookupAdjacent> {
    @inlinable public init(
        volume: Unidoc.VolumeSelector,
        vertex: Unidoc.VertexPath,
        layer: Unidoc.GroupLayer? = nil
    ) {
...
extension Unidoc.VertexQuery {
    private enum FacetOne: Mongo.MasterCodingModel {
    ...
    private enum FacetTwo: Mongo.MasterCodingModel {
        enum CodingKey: String {
...
extension Unidoc.VertexQuery: Mongo.PipelineQuery {
    public typealias Iteration = Mongo.Single<Unidoc.VertexOutput>

    @inlinable public var collation: Mongo.Collation { .casefolding }
    @inlinable public var from: Mongo.Collection? { Unidoc.DB.Volumes.name }
    @inlinable public var hint: Mongo.CollectionIndex? {
    ...
    private func build(facet: inout Mongo.FacetEncoder<FacetOne.CodingKey>) {
        facet[.principalOnly] {
            //  Exit early if there is no principal vertex.
            $0[stage: .unwind] = Output[.principalVertex]

            self.lookup.packages(
                &$0,
                volume: Output[.principalVolume],
    ...
    private func build(facet: inout Mongo.FacetEncoder<FacetTwo.CodingKey>) {
                        ...
                        $0[stage: .match] {
                            $0[.expr] {
                                $0[.and] {
                                    $0 {
                                        $0[.eq] = (
                                            Unidoc.SearchbotCell[
                                                .id
                                            ] / Unidoc.SearchbotCell.ID[.trunk],
                                            trunk
                                        )
                                    }
                                    $0 {
