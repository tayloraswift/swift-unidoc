...
extension Unidoc {
    ...
    @frozen public struct LookupAdjacent: Sendable {
        public let layer: GroupLayer?

        @inlinable init(layer: GroupLayer?) {
            self.layer = layer
        }
    }
}
extension Unidoc.LookupAdjacent: Unidoc.LookupContext {
    public static var lookupGridCell: Bool { true }
    ...
    public func groups(
        ...
        pipeline[stage: .lookup] {
            ...
            let local: LockedExtensions = .init(
                layer: self.layer,
                scope: "local",
                min: "min",
                max: "max"
            )
            let realm: LatestExtensions = .init(
                layer: self.layer,
                scope: "scope",
                id: "realm"
            )
            ...
            $0[.let] {
                ...
                guard
                case .default(let special) = special else {
                    return
                }
