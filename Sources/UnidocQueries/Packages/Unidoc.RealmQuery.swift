...
extension Unidoc {
    @frozen public struct RealmQuery: Equatable, Hashable, Sendable {
        public let symbol: String

        @usableFromInline let user: Unidoc.Account?

        @inlinable public init(realm symbol: String, user: Unidoc.Account? = nil) {
            self.symbol = symbol
            self.user = user
        }
    }
...
extension Unidoc.RealmQuery: Unidoc.AliasingQuery {
    ...
    public typealias CollectionOrigin = Unidoc.DB.RealmAliases
    public typealias CollectionTarget = Unidoc.DB.Realms

    @inlinable public static var target: Mongo.AnyKeyPath { Output[.metadata] }
