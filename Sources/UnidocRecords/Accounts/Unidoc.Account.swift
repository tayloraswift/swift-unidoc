...
{
    @frozen public
    struct Account:Equatable, Hashable, Sendable
    {
        public
        let type:Unidoc.AccountType
        @usableFromInline
        let user:UInt32

        @inlinable public
        init(type:Unidoc.AccountType, user:UInt32)
        {
            self.type = type
            self.user = user
        }
...
extension Unidoc.Account
{
    ...
    {
        guard
        let type:Unidoc.AccountType = .init(rawValue: bits >> 32)
        else
        {
            return nil
        }
...
{
    @inlinable public
    init?(rawValue:Int64) { self.init(bits: .init(bitPattern: rawValue)) }

    @inlinable public
    var rawValue:Int64 { .init(bitPattern: self.bits) }
}
...
{
    @inlinable public
    var description:String { "\(self.bits)" }
}
