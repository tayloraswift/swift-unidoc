...
{
    @frozen public
    struct BuildPayload:Sendable
    {
        public
        let metadata:SymbolGraphMetadata
        public
        let zlib:ArraySlice<UInt8>
        /// The uncompressed size, in bytes.
        public
        let size:Int64

        @inlinable public
        init(metadata:SymbolGraphMetadata, zlib:ArraySlice<UInt8>, size:Int64)
        {
...
extension Unidoc.BuildPayload:BSONDocumentDecodable
{
    ...
    init(bson:BSON.DocumentDecoder<CodingKey>) throws
    {
        self.init(metadata: try bson[.metadata].decode(),
            zlib: try bson[.zlib].decode(as: BSON.BinaryView<ArraySlice<UInt8>>.self,
                with: \.bytes),
