...
extension Markdown {
    @frozen public struct Snippet {
        public let id: Int32

        private let captionParser: any Markdown.ParsingEngine
        private let captionSource: Markdown.Source?

        public let slices: OrderedDictionary<String, SnippetSlice<Symbol.USR>>
...
extension Markdown.Snippet {
    public init(
        ...
        self.init(
            ...
            captionSource: caption.allSatisfy(\.isWhitespace) ? nil : .init(
                file: id,
                text: caption
            ),
            slices: index
