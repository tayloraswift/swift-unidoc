...
extension Markdown {
    @rethrows protocol SemanticPrefix {
...
extension Markdown.SemanticPrefix {
    ...
    /// a non-nil pattern.
    static func extract(from blocks: inout [Markdown.BlockElement]) rethrows -> Self? {
        guard case (let paragraph as Markdown.BlockParagraph)? = blocks.first else {
            return nil
        }
        guard let pattern: Self = try .extract(from: &paragraph.elements) else {
            return nil
        }
    ...
    private static func extract(
        from elements: inout [Markdown.InlineElement]
    ) rethrows -> Self? {
        for (index, span): (Int, Markdown.InlineElement) in zip(
            ...
            if  case .text(let text) = span,
                ...
                guard let pattern: Self = try .init(from: outer) else {
                    return nil
                }
