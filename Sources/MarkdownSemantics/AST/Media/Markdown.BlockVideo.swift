...
extension Markdown {
    final class BlockVideo: BlockContainer<BlockElement> {
        var source: SourceReference<Markdown.Source>?

        private(set) var poster: Outlinable<SourceString>?
        /// Not to be confused with ``source``.
        private(set) var src: Outlinable<SourceString>?
...
extension Markdown.BlockVideo: Markdown.BlockDirectiveType {
    ...
    func configure(option: Option, value: Markdown.SourceString) throws {
        switch option {
        case .poster:
            guard case nil = self.poster else {
