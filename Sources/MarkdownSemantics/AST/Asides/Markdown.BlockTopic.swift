...
{
    public final
    class BlockTopic:BlockElement
    {
        public private(set)
        var items:[Outlinable<InlineAutolink>]

        private
        init(items:[Outlinable<InlineAutolink>])
        {
            self.items = items
        }
...
{
    convenience
    init?(from list:__shared Markdown.BlockListUnordered)
    {
