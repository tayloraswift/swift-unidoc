...
struct SnippetParser {
    private let sourcemap: SourceLocationConverter

    private var captionText: String
    private var captionLine: Bool

    private var complete: [SliceBounds]

    /// The start of the current subslice. This is nil if the current slice is hidden or
    /// terminated.
    private var subslice: AbsolutePosition?
    private var current: SliceBounds
    private var currentLine: (indent: Int, gap: Range<AbsolutePosition>)?
...
extension SnippetParser {
    mutating func visit(token: TokenSyntax) {
    ...
    private mutating func visit(trivia: Trivia, at position: AbsolutePosition) {
        ...
        for piece: TriviaPiece in trivia {
            ...
            let currentLine: (
                indent: Int,
                gap: Range<AbsolutePosition>
            ) = self.currentLine else {
            ...
            guard
            let i: String.Index = token.index(
                token.startIndex,
                offsetBy: skip,
    ...
    mutating func push(marker: SliceMarker) {
...
extension SnippetParser {
    consuming func finish(at position: AbsolutePosition, in utf8: [UInt8]) -> (
        String,
        [Slice]
    ) {
    ...
    private consuming func finish(at position: AbsolutePosition) -> (String, [SliceBounds]) {
        if  let subslice: AbsolutePosition = self.subslice {
