...
extension Markdown {
    @frozen public struct SwiftLanguage {
        @usableFromInline let index: (any IndexStore)?

        @inlinable init(index: (any IndexStore)? = nil) {
            self.index = index
        }
    }
}
extension Markdown.SwiftLanguage: Markdown.CodeLanguageType {
    @inlinable public var name: String { "swift" }

    @inlinable public var highlighter: Highlighter { .init() }
}
extension Markdown.SwiftLanguage {
    public func parse(
        ...
        var parser: SnippetParser = .init(
            sourcemap: .init(fileName: "", tree: parsed),
    ...
    public func parse(
        code utf8: [UInt8],
        diff: [(Range<Int>, Markdown.DiffType?)]
    ) -> Markdown.Bytecode {
        .init {
