...
extension Markdown {
    @frozen public struct Parser<Flavor> where Flavor: Markdown.ParsingFlavor {
        private let plugins: [String: any Markdown.CodeLanguageType]
        private let `default`: (any Markdown.CodeLanguageType)?

        private var errors: [(any Error, at: SourceReference<Source>)] = []
...
extension Markdown.Parser: Markdown.ParsingEngine {
    public func parse(
        ...
        onError: (any Error, SourceReference<Markdown.Source>) -> ()
    ) -> [Markdown.BlockElement] {
        let document: _Document = .init(
            parsing: source.text, options: [
        ...
        for child: any _BlockMarkup in document.blockChildren {
            guard
            let block: Markdown.BlockElement = _copy.parse(block: child, from: source) else {
                continue
            }
...
extension Markdown.Parser {
    private mutating func parse(
        block markup:/* borrowing */ any _BlockMarkup,
        from source: Markdown.Source
    ) -> Markdown.BlockElement? {
        switch /* copy */ markup {
        case let block as _BlockQuote:
            return Markdown.BlockQuote.init(
                block.blockChildren.compactMap {
        ...
        case let block as _BlockDirective:
            guard
            let directive: any Markdown.BlockDirectiveType = Flavor[
                instantiating: block.name
            ] else {
                return nil
            }
            ...
            for child: any _BlockMarkup in block.blockChildren {
                guard
                let block: Markdown.BlockElement = self.parse(block: child, from: source) else {
                    continue
                }
        ...
        case let block as _Paragraph:
            return Markdown.BlockParagraph.init(
                block.inlineChildren.map {
                    Markdown.InlineElement.init(from: $0, in: source)
                }
            )

        case let table as _Table:
            return Markdown.Table.init(
                columns: table.columnAlignments.map {
                    switch $0 {
                    case nil:       nil
                    case .left?:    .left
                    case .center?:  .center
        ...
        case let block as _OrderedList:
            return Markdown.BlockListOrdered.init(
                block.listItems.map {
        ...
        case let block as _UnorderedList:
            return Markdown.BlockListUnordered.init(
                block.listItems.map {
                    self.parse(item: $0, from: source)
                }
            )
    ...
    private mutating func parse(
        ...
        from source: borrowing Markdown.Source
    ) -> Markdown.BlockItem {
        .init(
