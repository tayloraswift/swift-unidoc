...
extension Unidoc {
    struct InitCommand {
        @Argument var location: FilePath.Directory
...
extension Unidoc.InitCommand: AsyncParsableCommand {
    public static let configuration: CommandConfiguration = .init(commandName: "init")
    ...
    func run() async throws {
        ...
        var attempts: Int = 0
        waiting: do {
            async let interval: Void = Task.sleep(for: .seconds(1))
            do {
                try SystemProcess.init(
                    command: "docker",
                    "exec",
