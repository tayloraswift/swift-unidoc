@frozen public struct DiagnosticMessages {
    private var fragments: [DiagnosticFragment]

    public let status: DiagnosticLevel

    private init(fragments: [DiagnosticFragment], status: DiagnosticLevel) {
...
extension DiagnosticMessages {
    public mutating func demoteErrors(to level: DiagnosticLevel) {
    ...
    public func emit(colors: TerminalColors) {
        var text: String = ""
        self.write(to: &text, colors: colors)
        print(text)
    }

    private func write(to text: inout some TextOutputStream, colors: TerminalColors) {
        var first: Bool = true
