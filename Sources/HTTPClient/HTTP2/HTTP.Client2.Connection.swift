...
extension HTTP.Client2 {
    @frozen public struct Connection {
        @usableFromInline internal let channel: any Channel
        /// The hostname of the remote peer.
        public let remote: String

        @inlinable internal init(channel: any Channel, remote: String) {
...
extension HTTP.Client2.Connection: HTTP.ClientConnection {
    @inlinable public func buffer(string: Substring) -> ByteBuffer {
        self.channel.allocator.buffer(substring: string)
    }

    @inlinable public func buffer(bytes: ArraySlice<UInt8>) -> ByteBuffer {
        self.channel.allocator.buffer(bytes: bytes)
    }
}
extension HTTP.Client2.Connection {
    public func fetch(_ request: __owned HPACKHeaders) async throws -> HTTP.Client2.Facet {
        try await self.fetch(.init(headers: request, body: nil))
    }
    public func fetch(
        _ request: __owned HTTP.Client2.Request,
        timeout: Duration = .seconds(15)
    ) async throws -> HTTP.Client2.Facet {
        try await self.fetch(reducing: [request], into: .init(), timeout: timeout) { $0 = $1 }
    }

    public func fetch(
        _ batch: __owned [HTTP.Client2.Request]
    ) async throws -> [HTTP.Client2.Facet] {
        try await self.fetch(reducing: batch, into: []) { $0.append($1) }
    }

    @inlinable public func fetch<Response>(
        reducing batch: __owned [HTTP.Client2.Request],
        into initial: __owned Response,
        timeout: Duration = .seconds(15),
        with combine: (inout Response, HTTP.Client2.Facet) throws -> ()
