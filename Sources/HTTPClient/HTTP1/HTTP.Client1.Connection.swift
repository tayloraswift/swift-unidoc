...
extension HTTP.Client1 {
    @frozen public struct Connection {
        @usableFromInline let channel: any Channel
        /// The hostname of the remote peer.
        public let remote: String

        @inlinable init(channel: any Channel, remote: String) {
...
extension HTTP.Client1.Connection {
    ...
    //  buffer.
    public func fetch(
        _ request: __owned HTTP.Client1.Request,
        timeout: Duration = .seconds(15)
    ) async throws -> HTTP.Client1.Facet {
        try await withThrowingTaskGroup(of: HTTP.Client1.Facet.self) {
            ...
            guard
            let facet: HTTP.Client1.Facet = try await tasks.next() else {
                throw HTTP.RequestTimeoutError.init()
            }
