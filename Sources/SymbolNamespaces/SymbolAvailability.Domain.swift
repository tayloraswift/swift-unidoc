...
extension SymbolAvailability:JSONDecodable
{
    public
    ...
    init(json:JSON) throws
    {
            ...
            try [JSON.ObjectDecoder<CodingKeys>].init(json: json)
        {
            ...
            switch try json[.domain].decode(
                to: SymbolAvailability.DomainIdentifier.self)
            {
            case .all:
                self[nil] = .init(
            ...
            case .platform(let domain):
                self[domain] = .init(
                        ...
                        as: Bool.self)
                    {
                        ...
                        with: SymbolAvailability.DeprecatedMask.since(_:)),
                    introduced: try json[.introduced]?.decode(),
                    obsoleted: try json[.obsoleted]?.decode(),
                    renamed: renamed,
                    message: message)
