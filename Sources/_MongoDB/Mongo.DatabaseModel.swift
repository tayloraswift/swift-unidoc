...
extension Mongo {
    ...
    public protocol DatabaseModel: Identifiable<Database> {
...
extension Mongo.DatabaseModel {
    @inlinable public func query<Query>(
        with query: Query,
        on replica: Mongo.ReadPreference = .primary
    ) async throws -> Query.Iteration.Batch
        where Query: Mongo.PipelineQuery, Query.Iteration.Stride == Never {
        try await self.session.run(
            command: query.command(stride: nil),
            against: self.id,
            on: replica
        )
    }
    ...
    @discardableResult
    @inlinable public func update<Update>(
        with update: Update
    ) async throws -> Mongo.UpdateResponse<Update.Target.Element.ID>
...
extension Mongo.DatabaseModel {
    @inlinable public func observe<Source, Delta>(
        collection: Source,
        every interval: Milliseconds = .milliseconds(30_000),
        since start: inout BSON.Timestamp?,
        yield: (Mongo.ChangeEvent<Delta>) async throws -> ()
    ) async throws where
        ...
        try await self.session.run(
            command: Mongo.Aggregate<Mongo.Cursor<Mongo.ChangeEvent<Delta>>>.init(
                Source.name,
...
extension Mongo.DatabaseModel {
    ...
    @inlinable func explain<Command>(command: Command) async throws -> String
    ...
    @inlinable public func explain(query: some Mongo.PipelineQuery) async throws -> String {
        try await self.explain(command: query.command(stride: nil))
    }

    @inlinable public func explain(update: some Mongo.UpdateQuery) async throws -> String {
        try await self.explain(command: update.command)
    }

    @inlinable public func explain(
        find: Mongo.Find<some Mongo.ReadEffect>
    ) async throws -> String {
