...
extension Mongo {
    public protocol CollectionModel<Element> {
...
extension Mongo.CollectionModel where Capacity == Never {
    @inlinable public var capacity: Never { fatalError() }
...
extension Mongo.CollectionModel
    ...
    @inlinable public func find(last count: Int) async throws -> [Element] {
        try await session.run(
            command: Mongo.Find<Mongo.SingleBatch<Element>>.init(
                Self.name,
                limit: count
            ) {
...
extension Mongo.CollectionModel {
    ...
    private func setupIndexes(dropUnused: Bool = false) async throws {
    ...
    private func create(indexes: [String: Mongo.CollectionIndex]) async throws {
        if  indexes.isEmpty {
            return
        }
        ...
        do {
            ...
            response = try await session.run(
                command: Mongo.CreateIndexes.init(
                    Self.name,
    ...
    private func drop(indexes: [String]) async throws {
        if  indexes.isEmpty {
            return
        }
...
extension Mongo.CollectionModel where Element: BSONDocumentDecodable {
    @inlinable public func find(id: Element.ID) async throws -> Element? {
        try await self.find {
            $0[.filter] { $0["_id"] = id }
            $0[.hint] = "_id_"
        }
    ...
    @inlinable public func find(
        collation: Mongo.Collation? = nil,
        indexName: String? = nil,
        where predicate: (inout Mongo.PredicateEncoder) -> ()
    ...
    @inlinable public func find(
        by index: Mongo.CollectionIndex,
        where predicate: (inout Mongo.PredicateEncoder) -> ()
    ) async throws -> Element? {
    ...
    @inlinable func find(
        _ yield: (inout Mongo.Find<Mongo.Single<Element>>) -> ()
    ) async throws -> Element? {
        try await session.run(
...
extension Mongo.CollectionModel {
    public typealias Insertable = BSONDocumentEncodable & Identifiable<Element.ID>

    @inlinable func insert(some elements: some Sequence<some Insertable>) async throws {
        var count: Int = 0
    ...
    @inlinable func insert(some element: some Insertable) async throws {
        let response: Mongo.InsertResponse = try await self.session.run(
...
extension Mongo.CollectionModel {
    @inlinable func upsert(
        some elements: some Sequence<some Insertable>
    ) async throws -> Mongo.Updates<Element.ID> {
        var count: Int = 0
    ...
    @inlinable func upsert(some element: some Insertable) async throws -> Element.ID? {
        let response: Mongo.UpdateResponse<Element.ID> = try await self.session.run(
...
extension Mongo.CollectionModel where Element: Insertable {
    ...
    @inlinable public func insert(_ element: Element) async throws {
        try await self.insert(some: element)
    }

    /// Inserts multiple instances of this collection’s ``Element`` type, **none of which** are
    /// expected to already exist in the collection.
    @inlinable public func insert(_ elements: some Sequence<Element>) async throws {
    ...
    @discardableResult
    @inlinable public func replace(_ element: Element) async throws -> Bool? {
    ...
    @discardableResult
    @inlinable public func upsert(_ element: Element) async throws -> Element.ID? {
...
extension Mongo.CollectionModel where Element: BSONDecodable, Element.ID: BSONEncodable {
    @inlinable public func upsert(
        ...
        update: (inout Mongo.UpdateEncoder) -> ()
    ) async throws -> (state: Element, new: Bool) {
        let (element, upserted): (Element, Element.ID?) = try await session.run(
            command: Mongo.FindAndModify<Mongo.Upserting<Element, Element.ID>>.init(
                Self.name,
                returning: phase
            ) {
                $0[.query] { $0["_id"] = id }
    ...
    @inlinable public func modify(
        id: Element.ID,
        returning phase: Mongo.UpdatePhase = .new,
        update: (inout Mongo.UpdateEncoder) -> ()
    ) async throws -> Element? {
        let (element, _): (Element?, Never?) = try await session.run(
            command: Mongo.FindAndModify<Mongo.Existing<Element>>.init(
                Self.name,
                returning: phase
            ) {
                $0[.query] { $0["_id"] = id }
    ...
    @inlinable public func remove(id: Element.ID) async throws -> Element? {
        let (element, _): (Element?, Never?) = try await session.run(
            command: Mongo.FindAndModify<Mongo.Removing<Element>>.init(
                Self.name,
                returning: .deleted
            ) {
                $0[.query] { $0["_id"] = id }
...
extension Mongo.CollectionModel {
    ...
    @discardableResult
    @inlinable func update(some element: some Insertable) async throws -> Bool? {
        let response: Mongo.UpdateResponse<Element.ID> = try await session.run(
            command: Mongo.Update<Mongo.One, Element.ID>.init(Self.name) {
                $0.replace(element)
            },
    ...
    @inlinable public func update(
        do encode: (inout Mongo.UpdateListEncoder<Mongo.One>) throws -> ()
    ) async throws -> Bool? {
        let response: Mongo.UpdateResponse<Element.ID> = try await session.run(
            command: Mongo.Update<Mongo.One, Element.ID>.init(Self.name) {
                try encode(&$0)
    ...
    @inlinable public func updateMany(
        do encode: (inout Mongo.UpdateListEncoder<Mongo.Many>) throws -> ()
    ) async throws -> Int {
        let response: Mongo.UpdateResponse<Element.ID> = try await session.run(
...
extension Mongo.CollectionModel {
    ...
    @discardableResult
    @inlinable public func delete(id: Element.ID) async throws -> Bool {
    ...
    @inlinable public func deleteAll(
        collation: Mongo.Collation? = nil,
        indexName: String? = nil,
        where predicate: (inout Mongo.PredicateEncoder) -> ()
    ) async throws -> Int {
