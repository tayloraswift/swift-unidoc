extension Unidoc {
    @frozen public struct Edition: Equatable, Hashable, Sendable {
        public let package: Package
        public let version: Version

        @inlinable public init(package: Package, version: Version) {
...
extension Unidoc.Edition: CustomStringConvertible {
    @inlinable public var description: String { "\(self.package)[\(self.version)]" }
}
extension Unidoc.Edition {
    @inlinable public var min: Unidoc.Scalar { self + Int32.init(bitPattern: .min) }
    @inlinable public var max: Unidoc.Scalar { self + Int32.init(bitPattern: .max) }

    @inlinable public func contains(_ scalar: Unidoc.Scalar) -> Bool {
        scalar.package == self.package &&
        scalar.version == self.version
    }
}
extension Unidoc.Edition {
    @inlinable public static func + (self: Self, citizen: Int32) -> Unidoc.Scalar {
        .init(package: self.package, version: self.version, citizen: citizen)
    }

    @inlinable public static func - (scalar: Unidoc.Scalar, self: Self) -> Int32? {
