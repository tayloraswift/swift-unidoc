...
{
    public
    protocol ChartSource<Key, Sectors>
    {
...
{
    @inlinable public
    func chart(title:(Key, Key.ShareFormat) throws -> String) rethrows -> Pie.Chart<Key>
    {
        .init(shape: try self.shape(title: title))
    }

    @inlinable public
    func disc(title:(Key, Key.ShareFormat) throws -> String) rethrows -> Pie.Disc<Key>
    {
        .init(shape: try self.shape(title: title))
    }

    @inlinable
    func shape(title:(Key, Key.ShareFormat) throws -> String) rethrows -> Pie.Shape<Key>
    {
        ...
        for (key, value):(Key, Int) in sectors[..<last]
        {
            ...
            let slice:Pie.Shape<Key>.Slice = .init(geometry: .init(share: share,
                    from: start,
                    to: r),
