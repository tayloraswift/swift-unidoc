extension Dictionary where Key:Comparable, Value:Identifiable<Key>
{
    ...
    @inlinable public
    func orderingValuesTopologically(by edges:some Sequence<(Key, Key)>) -> [Value]?
    {
        ...
        return self.orderingValuesTopologically(from: sources,
            dependencies: dependencies,
            dependents: dependents)
    }

    @inlinable
    func orderingValuesTopologically(from sources:consuming [Value],
        dependencies:consuming [Key: Set<Key>],
        dependents:consuming [Key: [Value]]) -> [Value]?
    {
        ...
        while let source:Value = sources.popLast()
        {
            ...
            guard let next:[Value] = dependents.removeValue(forKey: source.id)
            else
            {
                continue
            }
