...
public final class TestGroup {
    public let context: TestContext

    public private(set) var passed: Int
    public private(set) var failed: [TestFailure]
...
extension TestGroup? {
    public static func / (self: TestGroup?, name: String) -> TestGroup? {
        self.flatMap { $0 / name }
    }
}
extension TestGroup {
...
extension TestGroup {
    ...
    public func `do`<Success>(
        ...
        do {
            ...
            self.failed.append(
                .init(
                    Assertion.ExpectedSuccess.init(caught: error),
                    location: .init(
                        function: function,
    ...
    public func `do`<Failure>(
        catching expected: Failure.Type,
        function: String = #function,
        file: String = #fileID,
        line: Int = #line,
        _ body: () throws -> ()
...
extension TestGroup {
    ...
    public func expect<Wrapped>(
        ...
        if  let value: Wrapped = optional {
            ...
            self.failed.append(
                .init(
                    Assertion.ExpectedValue<Wrapped>.init(),
                    location: .init(
                        function: function,
                        path: self.context.path,
                        file: file,
    ...
    public func expect<Wrapped>(
        ...
        if  let value: Wrapped = optional {
            self.failed.append(
                .init(
                    Assertion.ExpectedNil<Wrapped>.init(
                        unwrapped: value
                    ),
                    location: .init(
                        function: function,
