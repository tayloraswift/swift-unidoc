...
extension SSGC {
    ...
    struct OutlineResolver/*:~Copyable*/ {
        private let scopes: OutlineResolverEnvironment
...
extension SSGC.OutlineResolver {
    ...
    private var origin: Int32? { self.scopes.origin }

    private var codelinks: UCF.ProjectWideResolver {
        .init(
            scope: self.scopes.codelink,
            global: self.tables.packageLinks,
            causal: self.scopes.causalLinks
        )
    }

    private var doclinks: UCF.ArticleResolver {
...
extension SSGC.OutlineResolver {
    mutating func resolve(
        ...
        guard
        let selector: UCF.Selector = .init(renamed) else {
    ...
    mutating func translate(url: Markdown.SourceURL) -> SymbolGraph.Outline? {
        guard
        let translatable: UCF.Selector = url.translatableSelector else {
            return nil
        }
    ...
    mutating func outline(
        _ codelink: UCF.Selector,
        at source: SourceReference<Markdown.Source>
    ) -> SymbolGraph.Outline? {
        guard
        let target: (Int32, Int32?) = self.target(of: codelink, at: source) else {
            return nil
        }
    ...
    mutating func outline(
        ...
        if  doclink.path.isEmpty {
            guard
            let origin: Int32 = self.origin else {
            ...
            guard
            let codelink: UCF.Selector = .init(doclink.page) else {
            ...
            if  case nil = doclink.fragment,
                ...
                //  information!
                self.diagnostics[source] = SSGC.OutlineDiagnostic.suggestReformat(
                    doclink,
                    to: codelink
                )
            }
...
extension SSGC.OutlineResolver {
    private mutating func target(
        of codelink: UCF.Selector,
        at source: SourceReference<Markdown.Source>
    ) -> (Int32, Int32?)? {
        ...
        case .ambiguous(let overloads, rejected: let rejected):
            ...
            guard rejected.count == 1, overloads.isEmpty else {
