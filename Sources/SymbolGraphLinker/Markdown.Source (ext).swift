...
extension Markdown.Source {
    convenience init(
        comment: __shared /* borrowing */ SSGC.DocumentationComment,
        in file: Int32?
    ) {
...
extension Markdown.Source {
    @_spi(testable) public func parse(
        _: Markdown.SemanticDocument.Type = Markdown.SemanticDocument.self,
        markdownParser markdown: Markdown.Parser<Markdown.SwiftComment>,
        snippetsTable: [String: Markdown.Snippet],
        diagnostics: inout Diagnostics<SSGC.Symbolicator>
    ) -> Markdown.SemanticDocument {
        let blocks: [Markdown.BlockElement] = markdown.parse(self) {
            diagnostics[$1] = .warning($0)
        }

        var analyzer: Markdown.SemanticAnalyzer<SSGC.Symbolicator> = .init(
            consume diagnostics,
    ...
    @_spi(testable) public func parse(
        _: SSGC.Supplement.Type = SSGC.Supplement.self,
        markdownParser markdown: Markdown.Parser<Markdown.SwiftFlavor>,
        snippetsTable: [String: Markdown.Snippet],
        diagnostics: inout Diagnostics<SSGC.Symbolicator>
    ) throws -> SSGC.Supplement {
        let blocks: [Markdown.BlockElement] = markdown.parse(self) {
            diagnostics[$1] = .warning($0)
        }
        ...
        if  case (let tutorial as Markdown.BlockArticle)? = blocks.first {
            ...
            guard
            let headline: String = tutorial.headline, !headline.isEmpty else {
            ...
            return .init(
                type: tutorial is Markdown.TutorialIndex
                    ? .tutorials(headline)
                    : .tutorial(headline),
                body: document
            )
