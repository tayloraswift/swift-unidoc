...
extension SSGC.Linker {
    @_spi(testable) public struct Tables/*:~Copyable*/ {
        let definitions: [String: Void]
        var diagnostics: Diagnostics<SSGC.Symbolicator>

        @_spi(testable) public var packageLinks: UCF.ResolutionTable<UCF.PackageOverload>
        @_spi(testable) public var articleLinks: UCF.ArticleTable

        private(set) var anchors: SSGC.AnchorResolver

        private(set) var modules: [Symbol.Module: Int]

        private var articles: [Symbol.Article: Int32]
        private var decls: [Symbol.Decl: Int32]
        private var files: [Symbol.File: Int32]

        var graph: SymbolGraph

        @_spi(testable) public init(
...
extension SSGC.Linker.Tables {
    ...
    mutating func allocate(
        ...
        title: __owned Markdown.Bytecode
    ) -> Int32? {
        {
            if  case nil = $0 {
                let scalar: Int32 = self.graph.articles.append(
                    .init(headline: title),
                    id: article
                )
                $0 = scalar
    ...
    /// extensions.
    mutating func allocate(decl: SSGC.Decl, language: Phylum.Language) -> Int32? {
        {
            guard case nil = $0 else {
                return nil
            }

            let vertex: SymbolGraph.Decl = .init(
                language: language,
                phylum: decl.phylum,
                kinks: decl.kinks,
                path: decl.path
            ...
            let scalar: Int32 = self.graph.decls.append(
                .init(decl: vertex),
                id: decl.id
            )
            $0 = scalar
    ...
    @discardableResult
    mutating func allocate(decl: Symbol.Decl) -> Int32 {
        let scalar: Int32 = {
            switch $0 {
            case nil:
                let scalar: Int32 = self.graph.decls.append(
...
extension SSGC.Linker.Tables {
    func citizen(_ id: Symbol.Decl) -> Int32? {
        ...
        self.graph.decls.contains(citizen: scalar) else {
            return nil
        }
    ...
    mutating func intern(_ id: Symbol.Decl) -> Int32 {
        {
            switch $0 {
            case nil:
    ...
    mutating func intern(_ id: Symbol.File) -> Int32 {
        {
            switch $0 {
            case nil:
    ...
    mutating func intern(_ id: Symbol.Module) -> Int {
        {
            switch $0 {
            case nil:
...
extension SSGC.Linker.Tables {
    func resolve(
        binding: Markdown.InlineAutolink,
        ...
        guard
        let selector: UCF.Selector = .init(binding.text.string) else {
        ...
        let resolver: UCF.ProjectWideResolver = .init(
            scope: .init(
                namespace: namespace,
                imports: []
            ),
        ...
        case .module(let module):
            throw SSGC.SupplementBindingError.init(
                selector: selector,
        ...
        case .overload(let overload):
            guard case let overload as UCF.PackageOverload = overload else {
    ...
    mutating func index(normalizing article: Markdown.SemanticDocument, id scope: Int32?) {
        ...
        func rewrite(_ target: inout Markdown.InlineHyperlink.Target?) {
            ...
            case .url(url: let url)?:
                guard
                case "doc"? = url.scheme,
                case "#"? = url.suffix.string.first else {
                    return
                }
    ...
    mutating func inline(
        ...
        try sections.traverse {
            guard
            case let block as Markdown.BlockCodeReference = $0 else {
                return
            }

            guard
            let file: String = block.file else {
            ...
            guard
            let file: SSGC.Resource = resources[file] else {
...
extension SSGC.Linker.Tables {
    @_spi(testable) public mutating func resolving<Success>(
        with environment: SSGC.OutlineResolverEnvironment,
        do body: (inout SSGC.Outliner) throws -> Success
    ) rethrows -> Success {
    ...
    mutating func link(article: SSGC.Article, in context: SSGC.Linker.Context, at c: Int) {
        let environment: SSGC.OutlineResolverEnvironment = .init(
    ...
    mutating func link(
        ...
        if  let decl: SymbolGraph.Decl = self.graph.decls.nodes[id].decl {
            ...
            ((linked, topics), rename) = self.resolving(
                with: .init(
                    origin: id,
                    namespace: namespace,
                    context: context,
