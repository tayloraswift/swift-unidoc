...
extension SSGC {
    struct ResourceText {
        ...
        private var lines: [ArraySlice<UInt8>]
        let whole: [UInt8]

        private init(lines: [ArraySlice<UInt8>], whole: [UInt8]) {
            self.lines = lines
            self.whole = whole
        }
    }
}
extension SSGC.ResourceText {
    init(utf8: [UInt8], trimmingTrailingNewlines: Bool) {
        ...
        if  trimmingTrailingNewlines,
            let i: Int = self.lines.lastIndex(
                where: { $0.allSatisfy(whitespace.contains(_:)) }
            ) {
            self.lines[i...] = []
        }
    }
