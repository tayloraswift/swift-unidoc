...
extension SSGC {
    public struct Linker: ~Copyable {
        private let doccommentParser: Markdown.Parser<Markdown.SwiftComment>
        private let markdownParser: Markdown.Parser<Markdown.SwiftFlavor>
        private let swiftParser: Markdown.SwiftLanguage?

        private var contexts: [Context]
        private var snippets: [String: Markdown.Snippet]
        private var router: Router<FNV24?, Int32>
        private var tables: Tables

        private var supplements: [
            Int32: (source: Markdown.Source, body: Markdown.SemanticDocument)
        ]
        private var collations: ArticleCollations
...
extension SSGC.Linker {
    ...
    mutating func attach(
        ...
        for (offset, module): (Int, SSGC.ModuleIndex) in zip(self.contexts.indices, indexes) {
            guard
            case offset? = self.tables.modules[module.id] else {
    ...
    private mutating func allocateNodesFromExtensions(in indexes: [SSGC.ModuleIndex]) {
    ...
    private mutating func unfurlFeaturesFromExtensions(in indexes: [SSGC.ModuleIndex]) {
        for (offset, module): (Int, SSGC.ModuleIndex) in zip(self.contexts.indices, indexes) {
            self.unfurl(
    ...
    private mutating func colorizeReexportedDeclarations(in indexes: [SSGC.ModuleIndex]) {
...
extension SSGC.Linker {
    ...
    private mutating func allocate(
        declarations: [(id: Symbol.Module, decls: [SSGC.Decl])],
        ...
        ;
        {
            ...
            for (allocated, qualifier, decls):
                ...
                for (i, decl): (Int32, SSGC.Decl) in zip(allocated.range, decls) {
                    ...
                    self.tables.packageLinks[qualifier, decl.path].append(
                        .init(
                            traits: traits,
                            decl: i,
                            heir: nil,
    ...
    ) -> (ClosedRange<Int32>, [SSGC.Decl])? {
        var scalars: (first: Int32, last: Int32)? = nil
        let kept: [SSGC.Decl] = decls.filter {
            guard
            let scalar: Int32 = self.tables.allocate(decl: $0, language: language) else {
    ...
    private mutating func unfurl(
        ...
        self.contexts[offset].extensions = extensions.map {
            ...
            for (f, id): (Int32, Symbol.Decl) in zip(features, $0.features) {
                guard
                let feature: SSGC.DeclAlias = featuresBySymbol[id] else {
                    continue
                }
            ...
            let index: Int = self.tables.graph.decls.nodes[extendee].push(
                .init(
...
extension SSGC.Linker {
    private mutating func attach(
        ...
        guard
        let swift: Markdown.SwiftLanguage = self.swiftParser else {
            return
        }
    ...
    private mutating func attach(markdown: [any SSGC.ResourceFile], at offset: Int) throws {
        ...
        for file: any SSGC.ResourceFile in markdown {
            if  let article: SSGC.Article = try self.attach(supplement: file, in: namespace) {
                ...
                guard case .standalone(id: let id) = article.type else {
                    continue
                }
    ...
    private mutating func attach(
        supplement: any SSGC.ResourceFile,
        ...
        let source: Markdown.Source = .init(
            file: file,
...
extension SSGC.Linker {
    ...
    private mutating func collate(decl: SSGC.Decl, at i: Int32, context: Context) throws {
        ...
        case (let comment?, let supplement?):
            ...
            article = .init(
                combined: body.merged(appending: supplement.body),
                scope: scope,
    ...
    private mutating func collate(extension: SSGC.Extension, at i: Int32, index j: Int) throws {
        ...
        guard
        let comment: SSGC.DocumentationComment else {
            return
        }
        ...
        let collation: SSGC.ArticleCollation = .init(
            combined: markdown.parse(
                markdownParser: self.doccommentParser,
                snippetsTable: self.snippets,
                diagnostics: &self.tables.diagnostics
            ),
        ...
        try self.tables.inline(
            resources: self.contexts[c].resources,
...
extension SSGC.Linker {
    mutating func link() -> SymbolGraph {
        for (offset, context): (Int, Context) in zip(self.contexts.indices, self.contexts) {
            ...
            for (_, i, j): (SSGC.Extension, Int32, Int) in context.extensions {
                guard
                let article: SSGC.ArticleCollation = self.collations.move(i, j) else {
                    continue
                }
