...
extension Unidoc.DB.Packages {
    public static let indexAccount: Mongo.CollectionIndex = .init(
        "RepoAccount",
        unique: false
    ) {
    ...
    public static let indexRepoCreated: Mongo.CollectionIndex = .init(
        "RepoCreated",
        unique: false
    ) {
    ...
    public static let indexRepoGitHub: Mongo.CollectionIndex = .init(
        "RepoGitHub",
        unique: true
    ) {
    ...
    public static let indexRealm: Mongo.CollectionIndex = .init("Realm", unique: false) {
...
extension Unidoc.DB.Packages: Mongo.CollectionModel {
    public typealias Element = Unidoc.PackageMetadata

    @inlinable public static var name: Mongo.Collection { "Packages" }
...
extension Unidoc.DB.Packages {
    public func set(
        settings: Unidoc.PackageSettings,
        of package: Unidoc.Package
    ) async throws -> Unidoc.PackageMetadata? {
...
extension Unidoc.DB.Packages {
    ...
    public func detachWebhook(package: Unidoc.Package) async throws -> Unidoc.PackageMetadata? {
        try await self.modify(id: package) {
            $0[.unset] {
    ...
    func scan() async throws -> Unidoc.TextResource<Unidoc.DB.Metadata.Key> {
        ...
        let json: JSON = try await .array {
            ...
            try await session.run(
                command: Mongo.Find<Mongo.Cursor<Unidoc.PackageMetadata>>.init(
                    Self.name,
                    stride: 1024
                ) {
                    $0[.filter] {
