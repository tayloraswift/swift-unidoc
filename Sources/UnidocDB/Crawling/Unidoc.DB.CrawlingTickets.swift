...
extension Unidoc.DB {
    @frozen public struct CrawlingTickets {
...
extension Unidoc.DB.CrawlingTickets {
    public static let indexTime: Mongo.CollectionIndex = .init(
        "Time",
        unique: false
    ) {
...
extension Unidoc.DB.CrawlingTickets: Mongo.CollectionModel {
    public typealias Element = Unidoc.CrawlingTicket<Unidoc.Package>

    @inlinable public static var name: Mongo.Collection { "CrawlingTickets" }

    @inlinable public static var indexes: [Mongo.CollectionIndex] { [ Self.indexTime ] }
}
extension Unidoc.DB.CrawlingTickets {
    ...
    public func create(tickets: [Element]) async throws -> Mongo.Updates<Unidoc.Package> {
    ...
    public func find(stalest limit: Int) async throws -> [Element] {
        let command: Mongo.Find<Mongo.SingleBatch<Element>> = .init(
            Self.name,
            limit: limit
        ) {
