...
extension Unidoc.DB {
    @frozen public struct CrawlingWindows {
...
extension Unidoc.DB.CrawlingWindows {
    public static let indexExpiration: Mongo.CollectionIndex = .init(
        "LastCrawled",
        unique: false
    ) {
...
extension Unidoc.DB.CrawlingWindows: Mongo.CollectionModel {
    public typealias Element = Unidoc.CrawlingWindow

    @inlinable public static var name: Mongo.Collection { "CrawlingWindows" }

    @inlinable public static var indexes: [Mongo.CollectionIndex] { [ Self.indexExpiration ] }
}
extension Unidoc.DB.CrawlingWindows {
    ...
    public func create(previous days: Days) async throws -> Mongo.Updates<UnixMillisecond> {
    ...
    public func pull() async throws -> Unidoc.CrawlingWindow? {
        let command: Mongo.Find<Mongo.Single<Unidoc.CrawlingWindow>> = .init(
            Self.name,
            limit: 1
        ) {
