...
extension Unidoc.DB.Volumes {
    public static let indexCoordinatePatch: Mongo.CollectionIndex = .init(
        "CoordinatePatch",
        unique: true
    ) {
    ...
    public static let indexSymbolicPatch: Mongo.CollectionIndex = .init(
        "SymbolicPatch",
    ...
    public static let indexSymbolic: Mongo.CollectionIndex = .init(
        "Symbolic",
        collation: .casefolding,
        unique: true
    ) {
    ...
    public static let indexLatestFlag: Mongo.CollectionIndex = .init(
        "LatestFlag",
        unique: true
    ) {
...
extension Unidoc.DB.Volumes: Mongo.CollectionModel {
    public typealias Element = Unidoc.VolumeMetadata

    @inlinable public static var name: Mongo.Collection { "Volumes" }
...
extension Unidoc.DB.Volumes {
    func find(named symbol: Symbol.Volume) async throws -> Unidoc.VolumeMetadata? {
        try await session.run(
            command: Mongo.Find<Mongo.Single<Unidoc.VolumeMetadata>>.init(
                Self.name,
                limit: 1
            ) {
...
extension Unidoc.DB.Volumes {
    ...
    ) -> Mongo.Find<Mongo.SingleBatch<PatchView>> {
