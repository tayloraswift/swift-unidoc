...
extension Unidoc.DB {
    @frozen public struct Snapshots {
...
extension Unidoc.DB.Snapshots {
    public static let indexSwiftReleases: Mongo.CollectionIndex = .init(
        "SwiftReleases",
        unique: true
    ) {
    ...
    public static let indexSymbolGraphABI: Mongo.CollectionIndex = .init(
        "ABI/2",
        unique: false
    ) {
    ...
    public static let indexPendingActions: Mongo.CollectionIndex = .init(
        "Uplinking",
        unique: false
    ) {
...
extension Unidoc.DB.Snapshots: Mongo.CollectionModel {
    public typealias Element = Unidoc.Snapshot

    @inlinable public static var name: Mongo.Collection { "Snapshots" }
...
extension Unidoc.DB.Snapshots {
    ...
    func findAll(of pins: [Unidoc.Edition]) async throws -> [Unidoc.Snapshot] {
        try await session.run(
            command: Mongo.Find<Mongo.Cursor<Unidoc.Snapshot>>.init(
                Self.name,
                stride: 50,
                limit: 50
            ) {
...
extension Unidoc.DB.Snapshots {
    @discardableResult
    public func queueAll(for action: Unidoc.LinkerAction) async throws -> Int {
    ...
    public func queue(
        id: Unidoc.Edition,
        for action: Unidoc.LinkerAction
    ) async throws -> Bool? {
    ...
    @discardableResult
    public func clear(id: Unidoc.Edition) async throws -> Bool? {
        try await self.update {
            $0 {
    ...
    @discardableResult
    public func mark(id: Unidoc.Edition, vintage: Bool) async throws -> Bool? {
        try await self.update {
            $0 {
                $0[.q] { $0[Unidoc.Snapshot[.id]] = id }
                $0[.u] {
...
extension Unidoc.DB.Snapshots {
    ...
    public func pending(_ limit: Int) async throws -> [QueuedOperation] {
        try await session.run(
            command: Mongo.Find<Mongo.SingleBatch<QueuedOperation>>.init(
                Self.name,
                limit: limit
            ) {
                $0[.filter] {
    ...
    ) async throws -> [Unidoc.Edition] {
        let editions: [Mongo.IdentityDocument<Unidoc.Edition>] = try await session.run(
            command: Mongo.Find<
                Mongo.SingleBatch<Mongo.IdentityDocument<Unidoc.Edition>>
            >.init(
                Self.name,
                limit: limit
            ) {
