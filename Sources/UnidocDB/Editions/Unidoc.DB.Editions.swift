...
extension Unidoc.DB.Editions {
    public static let indexEditionName: Mongo.CollectionIndex = .init(
        "EditionName",
        collation: .simple,
        unique: true
    ) {
    ...
    public static let indexEditionCoordinate: Mongo.CollectionIndex = .init(
        "EditionCoordinate",
        unique: true
    ) {
        $0[Unidoc.EditionMetadata[.package]] = (-)
    ...
    public static let indexPrereleases: Mongo.CollectionIndex = .init(
        "Prereleases",
        unique: true
    ) {
        $0[Unidoc.EditionMetadata[.package]] = (-)
    ...
    public static let indexReleases: Mongo.CollectionIndex = .init(
        "Releases",
        unique: true
    ) {
        $0[Unidoc.EditionMetadata[.package]] = (-)
    ...
    public static let indexBranches: Mongo.CollectionIndex = .init(
        "Branches",
        unique: true
    ) {
        $0[Unidoc.EditionMetadata[.package]] = (+)
...
extension Unidoc.DB.Editions: Mongo.CollectionModel {
    public typealias Element = Unidoc.EditionMetadata

    @inlinable public static var name: Mongo.Collection { "Editions" }
