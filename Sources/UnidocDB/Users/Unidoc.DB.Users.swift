...
extension Unidoc.DB.Users {
    public static let indexRateLimit: Mongo.CollectionIndex = .init(
        "RateLimit/2",
        unique: false
    ) {
        $0[Unidoc.User[.apiLimitLeft]] = (+)
    }
    public static let indexAccess: Mongo.CollectionIndex = .init("Access", unique: false) {
...
extension Unidoc.DB.Users: Mongo.CollectionModel {
    public typealias Element = Unidoc.User

    @inlinable public static var name: Mongo.Collection { "Users" }
...
extension Unidoc.DB.Users {
    ...
    public func update(user: Unidoc.User) async throws -> Unidoc.UserSecrets {
        let (secrets, _): (Unidoc.UserSecrets, Unidoc.Account?) = try await session.run(
            command: Mongo.FindAndModify<
                Mongo.Upserting<Unidoc.UserSecrets, Unidoc.Account>
            >.init(
                Self.name,
                returning: .new
            ) {
    ...
    public func update(users: [Unidoc.User]) async throws -> Mongo.Updates<Unidoc.Account> {
    ...
    public func update(access: [Unidoc.Account], user: Unidoc.Account) async throws -> Bool? {
...
extension Unidoc.DB.Users {
    ...
    public func scramble(
        secret: Unidoc.User.CodingKey,
        user: Unidoc.Account
    ) async throws -> Unidoc.UserSecrets? {
        let (updated, _): (Unidoc.UserSecrets?, Never?) = try await session.run(
            command: Mongo.FindAndModify<Mongo.Existing<Unidoc.UserSecrets>>.init(
                Self.name,
                returning: .new
            ) {
                $0[.hint] {
...
extension Unidoc.DB.Users {
    ...
    public func charge(
        ...
        let (meter, _): (Unidoc.UserMeter?, Never?) = try await session.run(
            command: Mongo.FindAndModify<Mongo.Existing<Unidoc.UserMeter>>.init(
                Self.name,
                returning: .new
            ) {
                $0[.hint] {
    ...
    public func validate(user: Unidoc.UserSession) async throws -> Unidoc.UserRights? {
