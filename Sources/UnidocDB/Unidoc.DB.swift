...
@_spi(testable) import UnidocRecords
import UnixTime

extension Unidoc {
    @frozen public struct DB: Identifiable {
        public let session: Mongo.Session
        public let id: Mongo.Database

        public let settings: DatabaseSettings
...
extension Unidoc.DB {
    @inlinable public var searchbotCoverage: SearchbotCoverage {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var searchbotGrid: SearchbotGrid {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var crawlingTickets: CrawlingTickets {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var crawlingWindows: CrawlingWindows {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var pendingBuilds: PendingBuilds {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var completeBuilds: CompleteBuilds {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var repoFeed: RepoFeed {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var docsFeed: DocsFeed {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var realmAliases: RealmAliases {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var realms: Realms {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var packageAliases: PackageAliases {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var packages: Packages {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var editions: Editions {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var snapshots: Snapshots {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var sitemaps: Sitemaps {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var metadata: Metadata {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var packageDependencies: PackageDependencies {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var editionDependencies: EditionDependencies {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var volumes: Volumes {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var redirects: Redirects {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var vertices: Vertices {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var groups: Groups {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var search: Search {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var trees: Trees {
        .init(database: self.id, session: self.session)
    }

    @inlinable public var users: Users {
        .init(database: self.id, session: self.session)
    }
}
...
extension Unidoc.DB {
    ...
    public func index(realm: String) async throws -> (realm: Unidoc.RealmMetadata, new: Bool) {
    ...
    public func alias(
        ...
        guard
        let query: Unidoc.AliasQuery<PackageAliases> = .init(
            symbol: package,
            alias: alias
        ) else {
    ...
    public func index(
        ...
        if  let repo: Unidoc.PackageRepo,
            ...
            guard
            let modified: Unidoc.PackageMetadata else {
    ...
    public func index(
        ...
        name: String,
        sha1: SHA1?
    ) async throws -> (edition: Unidoc.EditionMetadata, new: Bool) {
        //  Placement involves autoincrement, which is why this cannot be done in an update.
        ...
        switch consume placement {
        case .new(let id):
            let edition: Unidoc.EditionMetadata = .init(
                id: .init(
                    package: package,
                    version: id
                ),
...
extension Unidoc.DB {
    public func label(docs: SymbolGraphMetadata) async throws -> (
        package: Unidoc.PackageMetadata,
        ...
        if  let commit: SymbolGraphMetadata.Commit = docs.commit {
            ...
            edition = .init(
                id: .init(package: package.id, version: -1),
                release: true,
                semver: .max,
                name: "__local",
                sha1: nil
...
extension Unidoc.DB {
    public func uplink(
        ...
        let linked: Unidoc.Mesh = try await self.link(
            snapshot,
    ...
    public func unlink(_ id: Unidoc.Edition) async throws -> Unidoc.UnlinkStatus? {
        guard
        let volume: Unidoc.VolumeMetadata = try await self.volumes.find(id: id) else {
            return nil
        }
    ...
    private func unlink(volume: Symbol.Volume) async throws -> Unidoc.UnlinkStatus? {
        guard
        let volume: Unidoc.VolumeMetadata = try await self.volumes.find(named: volume) else {
            return nil
        }
    ...
    @discardableResult
    private func unlink(volume: Unidoc.VolumeMetadata) async throws -> Bool {
...
extension Unidoc.DB {
    ...
    private func fillVolume(
        from mesh: consuming Unidoc.Mesh,
        hidden: Bool
    ) async throws -> Unidoc.SurfaceDelta? {
        ...
        let search: Unidoc.TextResource<Symbol.Volume> = .init(
            id: mesh.volume,
        ...
        if  let occupant: Unidoc.VolumeMetadata = try await self.volumes.find(
                named: mesh.volume
            ) {
            //  We should not clear release versions by name, only by coordinate.
            guard case nil = occupant.patch else {
                return nil
            }
        ...
        if  let latest: Unidoc.Edition = mesh.latestRelease {
            ...
            guard
            let realm: Unidoc.Realm = mesh.metadata.realm else {
                break alignment
            }
    ...
    private func pin(_ snapshot: Unidoc.Snapshot) async throws -> [Unidoc.EditionMetadata?] {
        print("pinning dependencies for \(snapshot.metadata.package.name)...")

        let local: Bool = snapshot.metadata.commit == nil

        guard
        let query: Unidoc.PinDependenciesQuery = .init(for: snapshot, locally: local) else {
            return .init(repeating: nil, count: snapshot.metadata.dependencies.count)
        }

        var pins: [Symbol.Package: Unidoc.EditionMetadata] = [:]

        for dependency: Symbol.PackageDependency<
                Unidoc.EditionMetadata
            > in try await self.query(
                with: query
            ) {
    ...
    private func link(
        _ snapshot: Unidoc.Snapshot,
        symbol package: Symbol.Package,
        linker: some Unidoc.GraphLinker,
        loader: some Unidoc.GraphLoader,
        realm: Unidoc.Realm?
    ) async throws -> Unidoc.Mesh {
        ...
        let mesh: Unidoc.Mesh = linker.link(
            ...
            dependencies: try await loader.load(
