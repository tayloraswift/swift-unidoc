...
extension Unidoc.DB {
    @frozen public struct PendingBuilds {
...
extension Unidoc.DB.PendingBuilds {
    public static let indexEnqueuedByHost: Mongo.CollectionIndex = .init(
        "EnqueuedByHost",
        unique: false
    ) {
    ...
    public static let indexEnqueued: Mongo.CollectionIndex = .init(
        "Enqueued/2",
        unique: false
    ) {
        $0[Unidoc.PendingBuild[.priority]] = (+)
    ...
    public static let indexLaunched: Mongo.CollectionIndex = .init("Launched", unique: false) {
        $0[Unidoc.PendingBuild[.launched]] = (+)
    } where: {
        $0[Unidoc.PendingBuild[.launched]] { $0[.exists] = true }
    }

    public static let indexPackage: Mongo.CollectionIndex = .init("Package", unique: false) {
        $0[Unidoc.PendingBuild[.package]] = (+)
    }
}
extension Unidoc.DB.PendingBuilds: Mongo.CollectionModel {
    public typealias Element = Unidoc.PendingBuild

    @inlinable public static var name: Mongo.Collection { "PendingBuilds" }
...
extension Unidoc.DB.PendingBuilds {
    public func selectBuild(
        await awaits: Bool,
        host: Symbol.Triple
    ) async throws -> Unidoc.PendingBuild? {
        //  Find a build, any build...
        if  let pendingBuild: Unidoc.PendingBuild = try await session.run(
                command: Mongo.Find<Mongo.Single<Unidoc.PendingBuild>>.init(
                    Self.name,
                    limit: 1
                ) {
                    $0[.filter] {
        ...
        return try await session.run(
            command: Mongo.Aggregate<
                Mongo.Cursor<
                    Mongo.ChangeEvent<
                        Unidoc.PendingBuildDelta
                    >
                >
            >.init(
                Self.name,
    ...
    public func submitBuild(
        ...
        let (pendingBuild, _): (Unidoc.PendingBuild, Bool) = try await self.upsert(id: id) {
            ...
            $0[.setOnInsert] = Unidoc.PendingBuild.init(
                id: id,
                run: now,
                priority: priority,
                enqueued: now,
    ...
    public func cancelBuild(id: Unidoc.Edition) async throws -> Bool {
    ...
    public func assignBuild(
        id: Unidoc.Edition,
        to assignee: Unidoc.Account
    ) async throws -> Bool {
        let (update, _): (Unidoc.PendingBuild?, Never?) = try await session.run(
            command: Mongo.FindAndModify<Mongo.Existing<Unidoc.PendingBuild>>.init(
                Self.name,
                returning: .new
            ) {
                $0[.query] {
    ...
    public func updateBuild(
        id: Unidoc.Edition,
        entered stage: Unidoc.BuildStage
    ) async throws -> Unidoc.PendingBuild? {
    ...
    public func finishBuild(id: Unidoc.Edition) async throws -> Unidoc.PendingBuild? {
    ...
    public func lintBuilds(startedBefore: UnixMillisecond) async throws -> Int {
    ...
    public func killBuilds(by assignee: Unidoc.Account) async throws -> Int {
        try await self.deleteAll {
            $0[Unidoc.PendingBuild[.launched]] { $0[.exists] = true }
            $0[Unidoc.PendingBuild[.assignee]] = assignee
        }
