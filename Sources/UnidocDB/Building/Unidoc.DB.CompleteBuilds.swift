...
extension Unidoc.DB {
    @frozen public struct CompleteBuilds {
...
extension Unidoc.DB.CompleteBuilds {
    ...
    public static let indexFinished: Mongo.CollectionIndex = .init("Finished") {
        $0[Unidoc.CompleteBuild[.finished]] = (-)
    }

    public static let indexFinishedByPackage: Mongo.CollectionIndex = .init(
        "FinishedByPackage"
    ) {
...
extension Unidoc.DB.CompleteBuilds: Mongo.CollectionModel {
    public typealias Element = Unidoc.CompleteBuild

    @inlinable public static var name: Mongo.Collection { "CompleteBuilds" }
