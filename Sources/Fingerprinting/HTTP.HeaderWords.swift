...
extension HTTP {
    @frozen @usableFromInline struct HeaderWords<Word, Separator> where Word: HeaderWord,
        Separator: HeaderWordSeparator {
        @usableFromInline var string: Substring
        @usableFromInline var index: String.Index

        @inlinable init(string: Substring) {
            self.string = string
...
extension HTTP.HeaderWords: IteratorProtocol {
    @inlinable mutating func next() -> Word? {
        while self.index < self.string.endIndex {
            let start: String.Index = self.index
            let slice: Substring

            if  let end: String.Index = self.string[start...].firstIndex(
