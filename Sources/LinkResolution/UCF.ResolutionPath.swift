...
extension UCF
{
    ...
    struct ResolutionPath:Equatable, Hashable, Sendable
    {
        @usableFromInline
        let string:String
...
extension UCF.ResolutionPath
{
    ...
    @inlinable
    static func join(_ namespace:Symbol.Module, _ path:UnqualifiedPath, _ last:String) -> Self
    {
        .init(string: "\(namespace) \(path.joined(separator: " ")) \(last)")
    }
    @inlinable
    static func join(_ namespace:Symbol.Module, _ path:UnqualifiedPath) -> Self
    {
        .init(string: "\(namespace) \(path.joined(separator: " "))")
    }
    @inlinable
    static func join(_ components:[String]) -> Self
    {
