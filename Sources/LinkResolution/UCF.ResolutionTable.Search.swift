...
extension UCF.ResolutionTable {
    struct Search {
        private let predicate: UCF.Predicate

        private var selected: [Symbol.Decl: Overload]
        private var rejected: [Symbol.Decl: Overload]
...
extension UCF.ResolutionTable.Search {
    mutating func add(_ candidates: InlineArray<Overload>) {
        ...
        for overload: Overload in candidates {
            guard self.predicate ~= overload.traits else {
    ...
    func any() -> UCF.Resolution<Overload>? {
        guard
        let overload: Overload = self.selected.values.first else {
            return nil
        }
    ...
    consuming func get() -> UCF.Resolution<Overload> {
