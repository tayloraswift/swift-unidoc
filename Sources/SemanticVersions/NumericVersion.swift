@frozen public enum NumericVersion: Equatable, Hashable, Comparable, Sendable {
...
extension NumericVersion {
    @inlinable public init(
        _ major: Int16,
        _ minor: UInt16?,
        _ patch: @autoclosure () throws -> UInt16?
    ) rethrows {
        if  let minor: UInt16 {
...
extension NumericVersion: LosslessStringConvertible {
    @inlinable public init?<String>(_ string: String) where String: StringProtocol {
        let components: [String.SubSequence] = string.split(
            separator: ".", maxSplits: 2,
            omittingEmptySubsequences: false
        )

        guard components.count > 0, let major: Int16 = .init(components[0]) else {
            return nil
        }
        ...
        guard let minor: UInt16 = .init(components[1]) else {
            return nil
        }
        ...
        guard components.count == 3, let patch: UInt16 = .init(components[2]) else {
            return nil
        }
        self = .patch(.v(major, minor, patch))
    }
