@frozen public struct PatchVersion: VectorVersion, Equatable, Hashable, Comparable, Sendable {
...
extension PatchVersion {
    ...
    @inlinable public static var max: Self { .v(.max, .max, .max) }

    /// Creates a semantic version with the given components.
    @inlinable public static func v(_ major: Int16, _ minor: UInt16, _ patch: UInt16) -> Self {
        self.init(components: .init(major: major, minor: minor, patch: patch))
    }

    @inlinable public init(padding version: NumericVersion, with fill: UInt16 = 0) {
        switch version {
...
extension PatchVersion {
    @inlinable public var major: MajorVersion { .v(self.components.major) }

    @inlinable public var minor: MinorVersion {
        .v(self.components.major, self.components.minor)
    }
}
